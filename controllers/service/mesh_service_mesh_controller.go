...
// MeshServiceMeshReconciler reconciles a MeshServiceMesh object
type MeshServiceMeshReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=service.alicloud.kubeform.com,resources=meshservicemeshes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=service.alicloud.kubeform.com,resources=meshservicemeshes/status,verbs=get;update;patch

func (r *MeshServiceMeshReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("meshservicemesh", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MeshServiceMesh")
...
func (r *MeshServiceMeshReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &servicev1alpha1.MeshServiceMesh{}); err != nil {
			klog.Error(err, "unable to set up auditor", servicev1alpha1.MeshServiceMesh{}.APIVersion, servicev1alpha1.MeshServiceMesh{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&servicev1alpha1.MeshServiceMesh{}).
