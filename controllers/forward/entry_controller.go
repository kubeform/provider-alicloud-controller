...
// +kubebuilder:rbac:groups=forward.alicloud.kubeform.com,resources=entries,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=forward.alicloud.kubeform.com,resources=entries/status,verbs=get;update;patch

func (r *EntryReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("entry", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Entry")
...
func (r *EntryReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &forwardv1alpha1.Entry{}); err != nil {
			klog.Error(err, "unable to set up auditor", forwardv1alpha1.Entry{}.APIVersion, forwardv1alpha1.Entry{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&forwardv1alpha1.Entry{}).
