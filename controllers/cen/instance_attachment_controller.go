...
// InstanceAttachmentReconciler reconciles a InstanceAttachment object
type InstanceAttachmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cen.alicloud.kubeform.com,resources=instanceattachments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cen.alicloud.kubeform.com,resources=instanceattachments/status,verbs=get;update;patch

func (r *InstanceAttachmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("instanceattachment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch InstanceAttachment")
...
func (r *InstanceAttachmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cenv1alpha1.InstanceAttachment{}); err != nil {
			klog.Error(err, "unable to set up auditor", cenv1alpha1.InstanceAttachment{}.APIVersion, cenv1alpha1.InstanceAttachment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cenv1alpha1.InstanceAttachment{}).
