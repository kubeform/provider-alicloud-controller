...
// FlowlogReconciler reconciles a Flowlog object
...
// +kubebuilder:rbac:groups=cen.alicloud.kubeform.com,resources=flowlogs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cen.alicloud.kubeform.com,resources=flowlogs/status,verbs=get;update;patch

func (r *FlowlogReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("flowlog", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Flowlog")
...
func (r *FlowlogReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cenv1alpha1.Flowlog{}); err != nil {
			klog.Error(err, "unable to set up auditor", cenv1alpha1.Flowlog{}.APIVersion, cenv1alpha1.Flowlog{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cenv1alpha1.Flowlog{}).
