...
// PrivateZoneReconciler reconciles a PrivateZone object
type PrivateZoneReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cen.alicloud.kubeform.com,resources=privatezones,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cen.alicloud.kubeform.com,resources=privatezones/status,verbs=get;update;patch

func (r *PrivateZoneReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("privatezone", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PrivateZone")
...
func (r *PrivateZoneReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cenv1alpha1.PrivateZone{}); err != nil {
			klog.Error(err, "unable to set up auditor", cenv1alpha1.PrivateZone{}.APIVersion, cenv1alpha1.PrivateZone{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cenv1alpha1.PrivateZone{}).
