...
// VbrHealthCheckReconciler reconciles a VbrHealthCheck object
type VbrHealthCheckReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cen.alicloud.kubeform.com,resources=vbrhealthchecks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cen.alicloud.kubeform.com,resources=vbrhealthchecks/status,verbs=get;update;patch

func (r *VbrHealthCheckReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vbrhealthcheck", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VbrHealthCheck")
...
func (r *VbrHealthCheckReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cenv1alpha1.VbrHealthCheck{}); err != nil {
			klog.Error(err, "unable to set up auditor", cenv1alpha1.VbrHealthCheck{}.APIVersion, cenv1alpha1.VbrHealthCheck{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cenv1alpha1.VbrHealthCheck{}).
