...
// BandwidthLimitReconciler reconciles a BandwidthLimit object
type BandwidthLimitReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cen.alicloud.kubeform.com,resources=bandwidthlimits,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cen.alicloud.kubeform.com,resources=bandwidthlimits/status,verbs=get;update;patch

func (r *BandwidthLimitReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("bandwidthlimit", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BandwidthLimit")
...
func (r *BandwidthLimitReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cenv1alpha1.BandwidthLimit{}); err != nil {
			klog.Error(err, "unable to set up auditor", cenv1alpha1.BandwidthLimit{}.APIVersion, cenv1alpha1.BandwidthLimit{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cenv1alpha1.BandwidthLimit{}).
