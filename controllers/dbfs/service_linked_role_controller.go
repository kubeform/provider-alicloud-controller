...
// +kubebuilder:rbac:groups=dbfs.alicloud.kubeform.com,resources=servicelinkedroles,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dbfs.alicloud.kubeform.com,resources=servicelinkedroles/status,verbs=get;update;patch

func (r *ServiceLinkedRoleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("servicelinkedrole", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServiceLinkedRole")
...
func (r *ServiceLinkedRoleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dbfsv1alpha1.ServiceLinkedRole{}); err != nil {
			klog.Error(err, "unable to set up auditor", dbfsv1alpha1.ServiceLinkedRole{}.APIVersion, dbfsv1alpha1.ServiceLinkedRole{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dbfsv1alpha1.ServiceLinkedRole{}).
