...
// +kubebuilder:rbac:groups=dfs.alicloud.kubeform.com,resources=mountpoints,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dfs.alicloud.kubeform.com,resources=mountpoints/status,verbs=get;update;patch

func (r *MountPointReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("mountpoint", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MountPoint")
...
func (r *MountPointReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dfsv1alpha1.MountPoint{}); err != nil {
			klog.Error(err, "unable to set up auditor", dfsv1alpha1.MountPoint{}.APIVersion, dfsv1alpha1.MountPoint{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dfsv1alpha1.MountPoint{}).
