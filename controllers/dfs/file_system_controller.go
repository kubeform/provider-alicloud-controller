...
// +kubebuilder:rbac:groups=dfs.alicloud.kubeform.com,resources=filesystems,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dfs.alicloud.kubeform.com,resources=filesystems/status,verbs=get;update;patch

func (r *FileSystemReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("filesystem", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FileSystem")
...
func (r *FileSystemReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dfsv1alpha1.FileSystem{}); err != nil {
			klog.Error(err, "unable to set up auditor", dfsv1alpha1.FileSystem{}.APIVersion, dfsv1alpha1.FileSystem{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dfsv1alpha1.FileSystem{}).
...
func (r *FileSystemReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for FileSystem")
			return nil
		}
		...
		resourceList := &dfsv1alpha1.FileSystemList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list FileSystem")
			return nil
		}
