...
// IndustrialPidProjectReconciler reconciles a IndustrialPidProject object
type IndustrialPidProjectReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=brain.alicloud.kubeform.com,resources=industrialpidprojects,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=brain.alicloud.kubeform.com,resources=industrialpidprojects/status,verbs=get;update;patch

func (r *IndustrialPidProjectReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("industrialpidproject", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IndustrialPidProject")
...
func (r *IndustrialPidProjectReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &brainv1alpha1.IndustrialPidProject{}); err != nil {
			klog.Error(err, "unable to set up auditor", brainv1alpha1.IndustrialPidProject{}.APIVersion, brainv1alpha1.IndustrialPidProject{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&brainv1alpha1.IndustrialPidProject{}).
