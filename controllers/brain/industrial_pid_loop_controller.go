...
// IndustrialPidLoopReconciler reconciles a IndustrialPidLoop object
type IndustrialPidLoopReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=brain.alicloud.kubeform.com,resources=industrialpidloops,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=brain.alicloud.kubeform.com,resources=industrialpidloops/status,verbs=get;update;patch

func (r *IndustrialPidLoopReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("industrialpidloop", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IndustrialPidLoop")
...
func (r *IndustrialPidLoopReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &brainv1alpha1.IndustrialPidLoop{}); err != nil {
			klog.Error(err, "unable to set up auditor", brainv1alpha1.IndustrialPidLoop{}.APIVersion, brainv1alpha1.IndustrialPidLoop{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&brainv1alpha1.IndustrialPidLoop{}).
