...
// UserPolicyAttachmentReconciler reconciles a UserPolicyAttachment object
type UserPolicyAttachmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ram.alicloud.kubeform.com,resources=userpolicyattachments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ram.alicloud.kubeform.com,resources=userpolicyattachments/status,verbs=get;update;patch

func (r *UserPolicyAttachmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("userpolicyattachment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch UserPolicyAttachment")
...
func (r *UserPolicyAttachmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ramv1alpha1.UserPolicyAttachment{}); err != nil {
			klog.Error(err, "unable to set up auditor", ramv1alpha1.UserPolicyAttachment{}.APIVersion, ramv1alpha1.UserPolicyAttachment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ramv1alpha1.UserPolicyAttachment{}).
