...
// LoginProfileReconciler reconciles a LoginProfile object
type LoginProfileReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ram.alicloud.kubeform.com,resources=loginprofiles,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ram.alicloud.kubeform.com,resources=loginprofiles/status,verbs=get;update;patch

func (r *LoginProfileReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("loginprofile", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LoginProfile")
...
func (r *LoginProfileReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ramv1alpha1.LoginProfile{}); err != nil {
			klog.Error(err, "unable to set up auditor", ramv1alpha1.LoginProfile{}.APIVersion, ramv1alpha1.LoginProfile{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ramv1alpha1.LoginProfile{}).
...
func (r *LoginProfileReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for LoginProfile")
			return nil
		}
		...
		resourceList := &ramv1alpha1.LoginProfileList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list LoginProfile")
			return nil
		}
