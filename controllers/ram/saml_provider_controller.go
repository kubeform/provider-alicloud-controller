...
// SamlProviderReconciler reconciles a SamlProvider object
...
// +kubebuilder:rbac:groups=ram.alicloud.kubeform.com,resources=samlproviders,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ram.alicloud.kubeform.com,resources=samlproviders/status,verbs=get;update;patch

func (r *SamlProviderReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("samlprovider", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SamlProvider")
...
func (r *SamlProviderReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ramv1alpha1.SamlProvider{}); err != nil {
			klog.Error(err, "unable to set up auditor", ramv1alpha1.SamlProvider{}.APIVersion, ramv1alpha1.SamlProvider{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ramv1alpha1.SamlProvider{}).
