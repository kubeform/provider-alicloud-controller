...
// SecurityPreferenceReconciler reconciles a SecurityPreference object
type SecurityPreferenceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ram.alicloud.kubeform.com,resources=securitypreferences,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ram.alicloud.kubeform.com,resources=securitypreferences/status,verbs=get;update;patch

func (r *SecurityPreferenceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("securitypreference", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SecurityPreference")
...
func (r *SecurityPreferenceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ramv1alpha1.SecurityPreference{}); err != nil {
			klog.Error(err, "unable to set up auditor", ramv1alpha1.SecurityPreference{}.APIVersion, ramv1alpha1.SecurityPreference{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ramv1alpha1.SecurityPreference{}).
