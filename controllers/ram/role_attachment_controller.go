...
// RoleAttachmentReconciler reconciles a RoleAttachment object
type RoleAttachmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ram.alicloud.kubeform.com,resources=roleattachments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ram.alicloud.kubeform.com,resources=roleattachments/status,verbs=get;update;patch

func (r *RoleAttachmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("roleattachment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RoleAttachment")
...
func (r *RoleAttachmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ramv1alpha1.RoleAttachment{}); err != nil {
			klog.Error(err, "unable to set up auditor", ramv1alpha1.RoleAttachment{}.APIVersion, ramv1alpha1.RoleAttachment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ramv1alpha1.RoleAttachment{}).
