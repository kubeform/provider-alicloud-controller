...
// RolePolicyAttachmentReconciler reconciles a RolePolicyAttachment object
type RolePolicyAttachmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ram.alicloud.kubeform.com,resources=rolepolicyattachments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ram.alicloud.kubeform.com,resources=rolepolicyattachments/status,verbs=get;update;patch

func (r *RolePolicyAttachmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("rolepolicyattachment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RolePolicyAttachment")
...
func (r *RolePolicyAttachmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ramv1alpha1.RolePolicyAttachment{}); err != nil {
			klog.Error(err, "unable to set up auditor", ramv1alpha1.RolePolicyAttachment{}.APIVersion, ramv1alpha1.RolePolicyAttachment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ramv1alpha1.RolePolicyAttachment{}).
