...
// EcsBackupClientReconciler reconciles a EcsBackupClient object
type EcsBackupClientReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=hbr.alicloud.kubeform.com,resources=ecsbackupclients,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=hbr.alicloud.kubeform.com,resources=ecsbackupclients/status,verbs=get;update;patch

func (r *EcsBackupClientReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ecsbackupclient", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EcsBackupClient")
...
func (r *EcsBackupClientReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &hbrv1alpha1.EcsBackupClient{}); err != nil {
			klog.Error(err, "unable to set up auditor", hbrv1alpha1.EcsBackupClient{}.APIVersion, hbrv1alpha1.EcsBackupClient{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&hbrv1alpha1.EcsBackupClient{}).
