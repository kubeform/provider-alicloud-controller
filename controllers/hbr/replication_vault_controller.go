...
// ReplicationVaultReconciler reconciles a ReplicationVault object
type ReplicationVaultReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=hbr.alicloud.kubeform.com,resources=replicationvaults,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=hbr.alicloud.kubeform.com,resources=replicationvaults/status,verbs=get;update;patch

func (r *ReplicationVaultReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("replicationvault", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ReplicationVault")
...
func (r *ReplicationVaultReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &hbrv1alpha1.ReplicationVault{}); err != nil {
			klog.Error(err, "unable to set up auditor", hbrv1alpha1.ReplicationVault{}.APIVersion, hbrv1alpha1.ReplicationVault{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&hbrv1alpha1.ReplicationVault{}).
