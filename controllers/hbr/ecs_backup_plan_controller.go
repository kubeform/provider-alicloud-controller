...
// EcsBackupPlanReconciler reconciles a EcsBackupPlan object
type EcsBackupPlanReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=hbr.alicloud.kubeform.com,resources=ecsbackupplans,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=hbr.alicloud.kubeform.com,resources=ecsbackupplans/status,verbs=get;update;patch

func (r *EcsBackupPlanReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ecsbackupplan", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EcsBackupPlan")
...
func (r *EcsBackupPlanReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &hbrv1alpha1.EcsBackupPlan{}); err != nil {
			klog.Error(err, "unable to set up auditor", hbrv1alpha1.EcsBackupPlan{}.APIVersion, hbrv1alpha1.EcsBackupPlan{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&hbrv1alpha1.EcsBackupPlan{}).
