...
// ServerBackupPlanReconciler reconciles a ServerBackupPlan object
type ServerBackupPlanReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=hbr.alicloud.kubeform.com,resources=serverbackupplans,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=hbr.alicloud.kubeform.com,resources=serverbackupplans/status,verbs=get;update;patch

func (r *ServerBackupPlanReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("serverbackupplan", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServerBackupPlan")
...
func (r *ServerBackupPlanReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &hbrv1alpha1.ServerBackupPlan{}); err != nil {
			klog.Error(err, "unable to set up auditor", hbrv1alpha1.ServerBackupPlan{}.APIVersion, hbrv1alpha1.ServerBackupPlan{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&hbrv1alpha1.ServerBackupPlan{}).
