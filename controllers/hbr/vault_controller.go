...
// +kubebuilder:rbac:groups=hbr.alicloud.kubeform.com,resources=vaults,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=hbr.alicloud.kubeform.com,resources=vaults/status,verbs=get;update;patch

func (r *VaultReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vault", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Vault")
...
func (r *VaultReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &hbrv1alpha1.Vault{}); err != nil {
			klog.Error(err, "unable to set up auditor", hbrv1alpha1.Vault{}.APIVersion, hbrv1alpha1.Vault{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&hbrv1alpha1.Vault{}).
