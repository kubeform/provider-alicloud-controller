...
// SubSubscriptionReconciler reconciles a SubSubscription object
type SubSubscriptionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=msc.alicloud.kubeform.com,resources=subsubscriptions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=msc.alicloud.kubeform.com,resources=subsubscriptions/status,verbs=get;update;patch

func (r *SubSubscriptionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("subsubscription", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SubSubscription")
...
func (r *SubSubscriptionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &mscv1alpha1.SubSubscription{}); err != nil {
			klog.Error(err, "unable to set up auditor", mscv1alpha1.SubSubscription{}.APIVersion, mscv1alpha1.SubSubscription{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&mscv1alpha1.SubSubscription{}).
