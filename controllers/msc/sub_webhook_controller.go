...
// SubWebhookReconciler reconciles a SubWebhook object
type SubWebhookReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=msc.alicloud.kubeform.com,resources=subwebhooks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=msc.alicloud.kubeform.com,resources=subwebhooks/status,verbs=get;update;patch

func (r *SubWebhookReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("subwebhook", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SubWebhook")
...
func (r *SubWebhookReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &mscv1alpha1.SubWebhook{}); err != nil {
			klog.Error(err, "unable to set up auditor", mscv1alpha1.SubWebhook{}.APIVersion, mscv1alpha1.SubWebhook{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&mscv1alpha1.SubWebhook{}).
