...
// SubContactReconciler reconciles a SubContact object
type SubContactReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=msc.alicloud.kubeform.com,resources=subcontacts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=msc.alicloud.kubeform.com,resources=subcontacts/status,verbs=get;update;patch

func (r *SubContactReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("subcontact", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SubContact")
...
func (r *SubContactReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &mscv1alpha1.SubContact{}); err != nil {
			klog.Error(err, "unable to set up auditor", mscv1alpha1.SubContact{}.APIVersion, mscv1alpha1.SubContact{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&mscv1alpha1.SubContact{}).
