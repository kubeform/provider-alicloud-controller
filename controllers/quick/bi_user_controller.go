...
// BiUserReconciler reconciles a BiUser object
type BiUserReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=quick.alicloud.kubeform.com,resources=biusers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=quick.alicloud.kubeform.com,resources=biusers/status,verbs=get;update;patch

func (r *BiUserReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("biuser", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BiUser")
...
func (r *BiUserReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &quickv1alpha1.BiUser{}); err != nil {
			klog.Error(err, "unable to set up auditor", quickv1alpha1.BiUser{}.APIVersion, quickv1alpha1.BiUser{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&quickv1alpha1.BiUser{}).
