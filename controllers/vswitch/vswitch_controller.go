...
// VswitchReconciler reconciles a Vswitch object
...
// +kubebuilder:rbac:groups=vswitch.alicloud.kubeform.com,resources=vswitches,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vswitch.alicloud.kubeform.com,resources=vswitches/status,verbs=get;update;patch

func (r *VswitchReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vswitch", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Vswitch")
...
func (r *VswitchReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vswitchv1alpha1.Vswitch{}); err != nil {
			klog.Error(err, "unable to set up auditor", vswitchv1alpha1.Vswitch{}.APIVersion, vswitchv1alpha1.Vswitch{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vswitchv1alpha1.Vswitch{}).
