...
// NasFileSystemReconciler reconciles a NasFileSystem object
type NasFileSystemReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ecd.alicloud.kubeform.com,resources=nasfilesystems,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ecd.alicloud.kubeform.com,resources=nasfilesystems/status,verbs=get;update;patch

func (r *NasFileSystemReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("nasfilesystem", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NasFileSystem")
...
func (r *NasFileSystemReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ecdv1alpha1.NasFileSystem{}); err != nil {
			klog.Error(err, "unable to set up auditor", ecdv1alpha1.NasFileSystem{}.APIVersion, ecdv1alpha1.NasFileSystem{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ecdv1alpha1.NasFileSystem{}).
