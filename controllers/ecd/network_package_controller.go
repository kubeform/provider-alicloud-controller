...
// NetworkPackageReconciler reconciles a NetworkPackage object
type NetworkPackageReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ecd.alicloud.kubeform.com,resources=networkpackages,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ecd.alicloud.kubeform.com,resources=networkpackages/status,verbs=get;update;patch

func (r *NetworkPackageReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("networkpackage", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NetworkPackage")
...
func (r *NetworkPackageReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ecdv1alpha1.NetworkPackage{}); err != nil {
			klog.Error(err, "unable to set up auditor", ecdv1alpha1.NetworkPackage{}.APIVersion, ecdv1alpha1.NetworkPackage{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ecdv1alpha1.NetworkPackage{}).
