...
// PolicyGroupReconciler reconciles a PolicyGroup object
...
// +kubebuilder:rbac:groups=ecd.alicloud.kubeform.com,resources=policygroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ecd.alicloud.kubeform.com,resources=policygroups/status,verbs=get;update;patch

func (r *PolicyGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("policygroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PolicyGroup")
...
func (r *PolicyGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ecdv1alpha1.PolicyGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", ecdv1alpha1.PolicyGroup{}.APIVersion, ecdv1alpha1.PolicyGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ecdv1alpha1.PolicyGroup{}).
