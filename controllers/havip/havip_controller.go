...
// HavipReconciler reconciles a Havip object
type HavipReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=havip.alicloud.kubeform.com,resources=havips,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=havip.alicloud.kubeform.com,resources=havips/status,verbs=get;update;patch

func (r *HavipReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("havip", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Havip")
...
func (r *HavipReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &havipv1alpha1.Havip{}); err != nil {
			klog.Error(err, "unable to set up auditor", havipv1alpha1.Havip{}.APIVersion, havipv1alpha1.Havip{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&havipv1alpha1.Havip{}).
