...
// SurveillanceSystemGroupReconciler reconciles a SurveillanceSystemGroup object
type SurveillanceSystemGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=video.alicloud.kubeform.com,resources=surveillancesystemgroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=video.alicloud.kubeform.com,resources=surveillancesystemgroups/status,verbs=get;update;patch

func (r *SurveillanceSystemGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("surveillancesystemgroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SurveillanceSystemGroup")
...
func (r *SurveillanceSystemGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &videov1alpha1.SurveillanceSystemGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", videov1alpha1.SurveillanceSystemGroup{}.APIVersion, videov1alpha1.SurveillanceSystemGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&videov1alpha1.SurveillanceSystemGroup{}).
