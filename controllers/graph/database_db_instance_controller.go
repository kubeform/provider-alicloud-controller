...
// DatabaseDbInstanceReconciler reconciles a DatabaseDbInstance object
type DatabaseDbInstanceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=graph.alicloud.kubeform.com,resources=databasedbinstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=graph.alicloud.kubeform.com,resources=databasedbinstances/status,verbs=get;update;patch

func (r *DatabaseDbInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("databasedbinstance", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DatabaseDbInstance")
...
func (r *DatabaseDbInstanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &graphv1alpha1.DatabaseDbInstance{}); err != nil {
			klog.Error(err, "unable to set up auditor", graphv1alpha1.DatabaseDbInstance{}.APIVersion, graphv1alpha1.DatabaseDbInstance{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&graphv1alpha1.DatabaseDbInstance{}).
