...
// +kubebuilder:rbac:groups=alikafka.alicloud.kubeform.com,resources=topics,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=alikafka.alicloud.kubeform.com,resources=topics/status,verbs=get;update;patch
...
func (r *TopicReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Topic")
...
func (r *TopicReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &alikafkav1alpha1.Topic{}); err != nil {
			klog.Error(err, "unable to set up auditor", alikafkav1alpha1.Topic{}.APIVersion, alikafkav1alpha1.Topic{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&alikafkav1alpha1.Topic{}).
