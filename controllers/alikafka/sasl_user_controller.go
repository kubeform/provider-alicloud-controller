...
// SaslUserReconciler reconciles a SaslUser object
type SaslUserReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=alikafka.alicloud.kubeform.com,resources=saslusers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=alikafka.alicloud.kubeform.com,resources=saslusers/status,verbs=get;update;patch

func (r *SaslUserReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("sasluser", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SaslUser")
...
func (r *SaslUserReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &alikafkav1alpha1.SaslUser{}); err != nil {
			klog.Error(err, "unable to set up auditor", alikafkav1alpha1.SaslUser{}.APIVersion, alikafkav1alpha1.SaslUser{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&alikafkav1alpha1.SaslUser{}).
...
func (r *SaslUserReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for SaslUser")
			return nil
		}
		...
		resourceList := &alikafkav1alpha1.SaslUserList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list SaslUser")
			return nil
		}
