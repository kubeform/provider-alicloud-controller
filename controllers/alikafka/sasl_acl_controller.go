...
// SaslACLReconciler reconciles a SaslACL object
type SaslACLReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=alikafka.alicloud.kubeform.com,resources=saslacls,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=alikafka.alicloud.kubeform.com,resources=saslacls/status,verbs=get;update;patch

func (r *SaslACLReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("saslacl", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SaslACL")
...
func (r *SaslACLReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &alikafkav1alpha1.SaslACL{}); err != nil {
			klog.Error(err, "unable to set up auditor", alikafkav1alpha1.SaslACL{}.APIVersion, alikafkav1alpha1.SaslACL{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&alikafkav1alpha1.SaslACL{}).
