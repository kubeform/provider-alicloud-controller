...
// HostAccountUserGroupAttachmentReconciler reconciles a HostAccountUserGroupAttachment object
type HostAccountUserGroupAttachmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=bastionhost.alicloud.kubeform.com,resources=hostaccountusergroupattachments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=bastionhost.alicloud.kubeform.com,resources=hostaccountusergroupattachments/status,verbs=get;update;patch

func (r *HostAccountUserGroupAttachmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("hostaccountusergroupattachment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch HostAccountUserGroupAttachment")
...
func (r *HostAccountUserGroupAttachmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &bastionhostv1alpha1.HostAccountUserGroupAttachment{}); err != nil {
			klog.Error(err, "unable to set up auditor", bastionhostv1alpha1.HostAccountUserGroupAttachment{}.APIVersion, bastionhostv1alpha1.HostAccountUserGroupAttachment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&bastionhostv1alpha1.HostAccountUserGroupAttachment{}).
