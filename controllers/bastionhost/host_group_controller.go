...
// +kubebuilder:rbac:groups=bastionhost.alicloud.kubeform.com,resources=hostgroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=bastionhost.alicloud.kubeform.com,resources=hostgroups/status,verbs=get;update;patch

func (r *HostGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("hostgroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch HostGroup")
...
func (r *HostGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &bastionhostv1alpha1.HostGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", bastionhostv1alpha1.HostGroup{}.APIVersion, bastionhostv1alpha1.HostGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&bastionhostv1alpha1.HostGroup{}).
