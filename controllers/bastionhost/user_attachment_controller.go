...
// UserAttachmentReconciler reconciles a UserAttachment object
type UserAttachmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=bastionhost.alicloud.kubeform.com,resources=userattachments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=bastionhost.alicloud.kubeform.com,resources=userattachments/status,verbs=get;update;patch

func (r *UserAttachmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("userattachment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch UserAttachment")
...
func (r *UserAttachmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &bastionhostv1alpha1.UserAttachment{}); err != nil {
			klog.Error(err, "unable to set up auditor", bastionhostv1alpha1.UserAttachment{}.APIVersion, bastionhostv1alpha1.UserAttachment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&bastionhostv1alpha1.UserAttachment{}).
