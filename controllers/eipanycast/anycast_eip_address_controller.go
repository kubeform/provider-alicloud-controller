...
// AnycastEipAddressReconciler reconciles a AnycastEipAddress object
type AnycastEipAddressReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=eipanycast.alicloud.kubeform.com,resources=anycasteipaddresses,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=eipanycast.alicloud.kubeform.com,resources=anycasteipaddresses/status,verbs=get;update;patch

func (r *AnycastEipAddressReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("anycasteipaddress", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AnycastEipAddress")
...
func (r *AnycastEipAddressReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &eipanycastv1alpha1.AnycastEipAddress{}); err != nil {
			klog.Error(err, "unable to set up auditor", eipanycastv1alpha1.AnycastEipAddress{}.APIVersion, eipanycastv1alpha1.AnycastEipAddress{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&eipanycastv1alpha1.AnycastEipAddress{}).
