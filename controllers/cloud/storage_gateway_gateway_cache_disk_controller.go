...
// StorageGatewayGatewayCacheDiskReconciler reconciles a StorageGatewayGatewayCacheDisk object
type StorageGatewayGatewayCacheDiskReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloud.alicloud.kubeform.com,resources=storagegatewaygatewaycachedisks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloud.alicloud.kubeform.com,resources=storagegatewaygatewaycachedisks/status,verbs=get;update;patch

func (r *StorageGatewayGatewayCacheDiskReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("storagegatewaygatewaycachedisk", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch StorageGatewayGatewayCacheDisk")
...
func (r *StorageGatewayGatewayCacheDiskReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudv1alpha1.StorageGatewayGatewayCacheDisk{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudv1alpha1.StorageGatewayGatewayCacheDisk{}.APIVersion, cloudv1alpha1.StorageGatewayGatewayCacheDisk{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudv1alpha1.StorageGatewayGatewayCacheDisk{}).
