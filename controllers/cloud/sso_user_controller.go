...
// SsoUserReconciler reconciles a SsoUser object
type SsoUserReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloud.alicloud.kubeform.com,resources=ssousers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloud.alicloud.kubeform.com,resources=ssousers/status,verbs=get;update;patch

func (r *SsoUserReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ssouser", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SsoUser")
...
func (r *SsoUserReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudv1alpha1.SsoUser{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudv1alpha1.SsoUser{}.APIVersion, cloudv1alpha1.SsoUser{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudv1alpha1.SsoUser{}).
...
func (r *SsoUserReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for SsoUser")
			return nil
		}
		...
		resourceList := &cloudv1alpha1.SsoUserList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list SsoUser")
			return nil
		}
