...
// FirewallControlPolicyOrderReconciler reconciles a FirewallControlPolicyOrder object
type FirewallControlPolicyOrderReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloud.alicloud.kubeform.com,resources=firewallcontrolpolicyorders,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloud.alicloud.kubeform.com,resources=firewallcontrolpolicyorders/status,verbs=get;update;patch

func (r *FirewallControlPolicyOrderReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("firewallcontrolpolicyorder", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FirewallControlPolicyOrder")
...
func (r *FirewallControlPolicyOrderReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudv1alpha1.FirewallControlPolicyOrder{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudv1alpha1.FirewallControlPolicyOrder{}.APIVersion, cloudv1alpha1.FirewallControlPolicyOrder{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudv1alpha1.FirewallControlPolicyOrder{}).
