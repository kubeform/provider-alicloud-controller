...
// SsoAccessConfigurationProvisioningReconciler reconciles a SsoAccessConfigurationProvisioning object
type SsoAccessConfigurationProvisioningReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloud.alicloud.kubeform.com,resources=ssoaccessconfigurationprovisionings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloud.alicloud.kubeform.com,resources=ssoaccessconfigurationprovisionings/status,verbs=get;update;patch

func (r *SsoAccessConfigurationProvisioningReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ssoaccessconfigurationprovisioning", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SsoAccessConfigurationProvisioning")
...
func (r *SsoAccessConfigurationProvisioningReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudv1alpha1.SsoAccessConfigurationProvisioning{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudv1alpha1.SsoAccessConfigurationProvisioning{}.APIVersion, cloudv1alpha1.SsoAccessConfigurationProvisioning{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudv1alpha1.SsoAccessConfigurationProvisioning{}).
