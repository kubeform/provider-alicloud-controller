...
// StorageGatewayGatewayReconciler reconciles a StorageGatewayGateway object
type StorageGatewayGatewayReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloud.alicloud.kubeform.com,resources=storagegatewaygateways,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloud.alicloud.kubeform.com,resources=storagegatewaygateways/status,verbs=get;update;patch

func (r *StorageGatewayGatewayReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("storagegatewaygateway", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch StorageGatewayGateway")
...
func (r *StorageGatewayGatewayReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudv1alpha1.StorageGatewayGateway{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudv1alpha1.StorageGatewayGateway{}.APIVersion, cloudv1alpha1.StorageGatewayGateway{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudv1alpha1.StorageGatewayGateway{}).
