...
// ConnectNetworkReconciler reconciles a ConnectNetwork object
type ConnectNetworkReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloud.alicloud.kubeform.com,resources=connectnetworks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloud.alicloud.kubeform.com,resources=connectnetworks/status,verbs=get;update;patch

func (r *ConnectNetworkReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("connectnetwork", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ConnectNetwork")
...
func (r *ConnectNetworkReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudv1alpha1.ConnectNetwork{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudv1alpha1.ConnectNetwork{}.APIVersion, cloudv1alpha1.ConnectNetwork{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudv1alpha1.ConnectNetwork{}).
