...
// +kubebuilder:rbac:groups=oos.alicloud.kubeform.com,resources=applicationgroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=oos.alicloud.kubeform.com,resources=applicationgroups/status,verbs=get;update;patch
...
func (r *ApplicationGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ApplicationGroup")
...
func (r *ApplicationGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &oosv1alpha1.ApplicationGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", oosv1alpha1.ApplicationGroup{}.APIVersion, oosv1alpha1.ApplicationGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&oosv1alpha1.ApplicationGroup{}).
