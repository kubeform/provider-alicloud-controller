...
// ServiceSettingReconciler reconciles a ServiceSetting object
type ServiceSettingReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=oos.alicloud.kubeform.com,resources=servicesettings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=oos.alicloud.kubeform.com,resources=servicesettings/status,verbs=get;update;patch

func (r *ServiceSettingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("servicesetting", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServiceSetting")
...
func (r *ServiceSettingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &oosv1alpha1.ServiceSetting{}); err != nil {
			klog.Error(err, "unable to set up auditor", oosv1alpha1.ServiceSetting{}.APIVersion, oosv1alpha1.ServiceSetting{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&oosv1alpha1.ServiceSetting{}).
