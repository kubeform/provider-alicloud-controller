...
// StateConfigurationReconciler reconciles a StateConfiguration object
type StateConfigurationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=oos.alicloud.kubeform.com,resources=stateconfigurations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=oos.alicloud.kubeform.com,resources=stateconfigurations/status,verbs=get;update;patch

func (r *StateConfigurationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("stateconfiguration", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch StateConfiguration")
...
func (r *StateConfigurationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &oosv1alpha1.StateConfiguration{}); err != nil {
			klog.Error(err, "unable to set up auditor", oosv1alpha1.StateConfiguration{}.APIVersion, oosv1alpha1.StateConfiguration{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&oosv1alpha1.StateConfiguration{}).
