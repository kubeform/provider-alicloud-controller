...
// SecretParameterReconciler reconciles a SecretParameter object
type SecretParameterReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=oos.alicloud.kubeform.com,resources=secretparameters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=oos.alicloud.kubeform.com,resources=secretparameters/status,verbs=get;update;patch

func (r *SecretParameterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("secretparameter", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SecretParameter")
...
func (r *SecretParameterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &oosv1alpha1.SecretParameter{}); err != nil {
			klog.Error(err, "unable to set up auditor", oosv1alpha1.SecretParameter{}.APIVersion, oosv1alpha1.SecretParameter{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&oosv1alpha1.SecretParameter{}).
...
func (r *SecretParameterReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for SecretParameter")
			return nil
		}
		...
		resourceList := &oosv1alpha1.SecretParameterList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list SecretParameter")
			return nil
		}
