...
// +kubebuilder:rbac:groups=oos.alicloud.kubeform.com,resources=parameters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=oos.alicloud.kubeform.com,resources=parameters/status,verbs=get;update;patch

func (r *ParameterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("parameter", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Parameter")
...
func (r *ParameterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &oosv1alpha1.Parameter{}); err != nil {
			klog.Error(err, "unable to set up auditor", oosv1alpha1.Parameter{}.APIVersion, oosv1alpha1.Parameter{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&oosv1alpha1.Parameter{}).
