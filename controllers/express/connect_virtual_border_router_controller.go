...
// ConnectVirtualBorderRouterReconciler reconciles a ConnectVirtualBorderRouter object
type ConnectVirtualBorderRouterReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=express.alicloud.kubeform.com,resources=connectvirtualborderrouters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=express.alicloud.kubeform.com,resources=connectvirtualborderrouters/status,verbs=get;update;patch

func (r *ConnectVirtualBorderRouterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("connectvirtualborderrouter", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ConnectVirtualBorderRouter")
...
func (r *ConnectVirtualBorderRouterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &expressv1alpha1.ConnectVirtualBorderRouter{}); err != nil {
			klog.Error(err, "unable to set up auditor", expressv1alpha1.ConnectVirtualBorderRouter{}.APIVersion, expressv1alpha1.ConnectVirtualBorderRouter{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&expressv1alpha1.ConnectVirtualBorderRouter{}).
