...
// ConnectPhysicalConnectionReconciler reconciles a ConnectPhysicalConnection object
type ConnectPhysicalConnectionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=express.alicloud.kubeform.com,resources=connectphysicalconnections,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=express.alicloud.kubeform.com,resources=connectphysicalconnections/status,verbs=get;update;patch

func (r *ConnectPhysicalConnectionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("connectphysicalconnection", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ConnectPhysicalConnection")
...
func (r *ConnectPhysicalConnectionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &expressv1alpha1.ConnectPhysicalConnection{}); err != nil {
			klog.Error(err, "unable to set up auditor", expressv1alpha1.ConnectPhysicalConnection{}.APIVersion, expressv1alpha1.ConnectPhysicalConnection{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&expressv1alpha1.ConnectPhysicalConnection{}).
