...
// +kubebuilder:rbac:groups=fnf.alicloud.kubeform.com,resources=flows,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=fnf.alicloud.kubeform.com,resources=flows/status,verbs=get;update;patch

func (r *FlowReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("flow", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Flow")
...
func (r *FlowReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &fnfv1alpha1.Flow{}); err != nil {
			klog.Error(err, "unable to set up auditor", fnfv1alpha1.Flow{}.APIVersion, fnfv1alpha1.Flow{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&fnfv1alpha1.Flow{}).
