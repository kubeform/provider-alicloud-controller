...
// +kubebuilder:rbac:groups=fnf.alicloud.kubeform.com,resources=executions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=fnf.alicloud.kubeform.com,resources=executions/status,verbs=get;update;patch

func (r *ExecutionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("execution", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Execution")
...
func (r *ExecutionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &fnfv1alpha1.Execution{}); err != nil {
			klog.Error(err, "unable to set up auditor", fnfv1alpha1.Execution{}.APIVersion, fnfv1alpha1.Execution{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&fnfv1alpha1.Execution{}).
