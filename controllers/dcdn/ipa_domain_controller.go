...
// IpaDomainReconciler reconciles a IpaDomain object
type IpaDomainReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dcdn.alicloud.kubeform.com,resources=ipadomains,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dcdn.alicloud.kubeform.com,resources=ipadomains/status,verbs=get;update;patch

func (r *IpaDomainReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ipadomain", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IpaDomain")
...
func (r *IpaDomainReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dcdnv1alpha1.IpaDomain{}); err != nil {
			klog.Error(err, "unable to set up auditor", dcdnv1alpha1.IpaDomain{}.APIVersion, dcdnv1alpha1.IpaDomain{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dcdnv1alpha1.IpaDomain{}).
