...
// AppAttachmentReconciler reconciles a AppAttachment object
type AppAttachmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=apigateway.alicloud.kubeform.com,resources=appattachments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apigateway.alicloud.kubeform.com,resources=appattachments/status,verbs=get;update;patch

func (r *AppAttachmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("appattachment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AppAttachment")
...
func (r *AppAttachmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &apigatewayv1alpha1.AppAttachment{}); err != nil {
			klog.Error(err, "unable to set up auditor", apigatewayv1alpha1.AppAttachment{}.APIVersion, apigatewayv1alpha1.AppAttachment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&apigatewayv1alpha1.AppAttachment{}).
