...
// VpcAccessReconciler reconciles a VpcAccess object
type VpcAccessReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=apigateway.alicloud.kubeform.com,resources=vpcaccesses,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apigateway.alicloud.kubeform.com,resources=vpcaccesses/status,verbs=get;update;patch

func (r *VpcAccessReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vpcaccess", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VpcAccess")
...
func (r *VpcAccessReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &apigatewayv1alpha1.VpcAccess{}); err != nil {
			klog.Error(err, "unable to set up auditor", apigatewayv1alpha1.VpcAccess{}.APIVersion, apigatewayv1alpha1.VpcAccess{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&apigatewayv1alpha1.VpcAccess{}).
