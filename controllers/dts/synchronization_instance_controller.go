...
// SynchronizationInstanceReconciler reconciles a SynchronizationInstance object
type SynchronizationInstanceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dts.alicloud.kubeform.com,resources=synchronizationinstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dts.alicloud.kubeform.com,resources=synchronizationinstances/status,verbs=get;update;patch

func (r *SynchronizationInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("synchronizationinstance", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SynchronizationInstance")
...
func (r *SynchronizationInstanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dtsv1alpha1.SynchronizationInstance{}); err != nil {
			klog.Error(err, "unable to set up auditor", dtsv1alpha1.SynchronizationInstance{}.APIVersion, dtsv1alpha1.SynchronizationInstance{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dtsv1alpha1.SynchronizationInstance{}).
