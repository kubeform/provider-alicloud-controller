...
// SubscriptionJobReconciler reconciles a SubscriptionJob object
type SubscriptionJobReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dts.alicloud.kubeform.com,resources=subscriptionjobs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dts.alicloud.kubeform.com,resources=subscriptionjobs/status,verbs=get;update;patch

func (r *SubscriptionJobReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("subscriptionjob", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SubscriptionJob")
...
func (r *SubscriptionJobReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dtsv1alpha1.SubscriptionJob{}); err != nil {
			klog.Error(err, "unable to set up auditor", dtsv1alpha1.SubscriptionJob{}.APIVersion, dtsv1alpha1.SubscriptionJob{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dtsv1alpha1.SubscriptionJob{}).
