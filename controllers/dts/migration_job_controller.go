...
// +kubebuilder:rbac:groups=dts.alicloud.kubeform.com,resources=migrationjobs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dts.alicloud.kubeform.com,resources=migrationjobs/status,verbs=get;update;patch

func (r *MigrationJobReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("migrationjob", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MigrationJob")
...
func (r *MigrationJobReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dtsv1alpha1.MigrationJob{}); err != nil {
			klog.Error(err, "unable to set up auditor", dtsv1alpha1.MigrationJob{}.APIVersion, dtsv1alpha1.MigrationJob{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dtsv1alpha1.MigrationJob{}).
