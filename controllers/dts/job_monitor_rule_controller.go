...
// JobMonitorRuleReconciler reconciles a JobMonitorRule object
type JobMonitorRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dts.alicloud.kubeform.com,resources=jobmonitorrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dts.alicloud.kubeform.com,resources=jobmonitorrules/status,verbs=get;update;patch

func (r *JobMonitorRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("jobmonitorrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch JobMonitorRule")
...
func (r *JobMonitorRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dtsv1alpha1.JobMonitorRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", dtsv1alpha1.JobMonitorRule{}.APIVersion, dtsv1alpha1.JobMonitorRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dtsv1alpha1.JobMonitorRule{}).
