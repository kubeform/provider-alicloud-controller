...
// MigrationInstanceReconciler reconciles a MigrationInstance object
type MigrationInstanceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dts.alicloud.kubeform.com,resources=migrationinstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dts.alicloud.kubeform.com,resources=migrationinstances/status,verbs=get;update;patch

func (r *MigrationInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("migrationinstance", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MigrationInstance")
...
func (r *MigrationInstanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dtsv1alpha1.MigrationInstance{}); err != nil {
			klog.Error(err, "unable to set up auditor", dtsv1alpha1.MigrationInstance{}.APIVersion, dtsv1alpha1.MigrationInstance{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dtsv1alpha1.MigrationInstance{}).
