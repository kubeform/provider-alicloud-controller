...
// CenterGroupReconciler reconciles a CenterGroup object
type CenterGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=security.alicloud.kubeform.com,resources=centergroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=security.alicloud.kubeform.com,resources=centergroups/status,verbs=get;update;patch

func (r *CenterGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("centergroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CenterGroup")
...
func (r *CenterGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &securityv1alpha1.CenterGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", securityv1alpha1.CenterGroup{}.APIVersion, securityv1alpha1.CenterGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&securityv1alpha1.CenterGroup{}).
