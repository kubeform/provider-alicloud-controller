...
// GroupRuleReconciler reconciles a GroupRule object
type GroupRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=security.alicloud.kubeform.com,resources=grouprules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=security.alicloud.kubeform.com,resources=grouprules/status,verbs=get;update;patch

func (r *GroupRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("grouprule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GroupRule")
...
func (r *GroupRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &securityv1alpha1.GroupRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", securityv1alpha1.GroupRule{}.APIVersion, securityv1alpha1.GroupRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&securityv1alpha1.GroupRule{}).
