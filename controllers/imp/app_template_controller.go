...
// AppTemplateReconciler reconciles a AppTemplate object
...
// +kubebuilder:rbac:groups=imp.alicloud.kubeform.com,resources=apptemplates,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=imp.alicloud.kubeform.com,resources=apptemplates/status,verbs=get;update;patch

func (r *AppTemplateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("apptemplate", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AppTemplate")
...
func (r *AppTemplateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &impv1alpha1.AppTemplate{}); err != nil {
			klog.Error(err, "unable to set up auditor", impv1alpha1.AppTemplate{}.APIVersion, impv1alpha1.AppTemplate{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&impv1alpha1.AppTemplate{}).
