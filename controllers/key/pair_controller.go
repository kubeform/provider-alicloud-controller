...
// PairReconciler reconciles a Pair object
...
// +kubebuilder:rbac:groups=key.alicloud.kubeform.com,resources=pairs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=key.alicloud.kubeform.com,resources=pairs/status,verbs=get;update;patch

func (r *PairReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("pair", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Pair")
...
func (r *PairReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &keyv1alpha1.Pair{}); err != nil {
			klog.Error(err, "unable to set up auditor", keyv1alpha1.Pair{}.APIVersion, keyv1alpha1.Pair{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&keyv1alpha1.Pair{}).
