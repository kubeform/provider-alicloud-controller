...
// PairAttachmentReconciler reconciles a PairAttachment object
type PairAttachmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=key.alicloud.kubeform.com,resources=pairattachments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=key.alicloud.kubeform.com,resources=pairattachments/status,verbs=get;update;patch

func (r *PairAttachmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("pairattachment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PairAttachment")
...
func (r *PairAttachmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &keyv1alpha1.PairAttachment{}); err != nil {
			klog.Error(err, "unable to set up auditor", keyv1alpha1.PairAttachment{}.APIVersion, keyv1alpha1.PairAttachment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&keyv1alpha1.PairAttachment{}).
