...
// KubernetesPermissionsReconciler reconciles a KubernetesPermissions object
type KubernetesPermissionsReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cs.alicloud.kubeform.com,resources=kubernetespermissions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cs.alicloud.kubeform.com,resources=kubernetespermissions/status,verbs=get;update;patch

func (r *KubernetesPermissionsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("kubernetespermissions", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch KubernetesPermissions")
...
func (r *KubernetesPermissionsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &csv1alpha1.KubernetesPermissions{}); err != nil {
			klog.Error(err, "unable to set up auditor", csv1alpha1.KubernetesPermissions{}.APIVersion, csv1alpha1.KubernetesPermissions{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&csv1alpha1.KubernetesPermissions{}).
