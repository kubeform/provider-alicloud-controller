...
// AutoscalingConfigReconciler reconciles a AutoscalingConfig object
type AutoscalingConfigReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cs.alicloud.kubeform.com,resources=autoscalingconfigs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cs.alicloud.kubeform.com,resources=autoscalingconfigs/status,verbs=get;update;patch

func (r *AutoscalingConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("autoscalingconfig", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AutoscalingConfig")
...
func (r *AutoscalingConfigReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &csv1alpha1.AutoscalingConfig{}); err != nil {
			klog.Error(err, "unable to set up auditor", csv1alpha1.AutoscalingConfig{}.APIVersion, csv1alpha1.AutoscalingConfig{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&csv1alpha1.AutoscalingConfig{}).
