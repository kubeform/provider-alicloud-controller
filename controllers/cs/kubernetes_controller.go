...
// +kubebuilder:rbac:groups=cs.alicloud.kubeform.com,resources=kubernetes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cs.alicloud.kubeform.com,resources=kubernetes/status,verbs=get;update;patch
...
func (r *KubernetesReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &csv1alpha1.Kubernetes{}); err != nil {
			klog.Error(err, "unable to set up auditor", csv1alpha1.Kubernetes{}.APIVersion, csv1alpha1.Kubernetes{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&csv1alpha1.Kubernetes{}).
...
func (r *KubernetesReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for Kubernetes")
			return nil
		}
		...
		resourceList := &csv1alpha1.KubernetesList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list Kubernetes")
			return nil
		}
