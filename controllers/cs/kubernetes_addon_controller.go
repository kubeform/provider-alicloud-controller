...
// KubernetesAddonReconciler reconciles a KubernetesAddon object
type KubernetesAddonReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cs.alicloud.kubeform.com,resources=kubernetesaddons,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cs.alicloud.kubeform.com,resources=kubernetesaddons/status,verbs=get;update;patch

func (r *KubernetesAddonReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("kubernetesaddon", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch KubernetesAddon")
...
func (r *KubernetesAddonReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &csv1alpha1.KubernetesAddon{}); err != nil {
			klog.Error(err, "unable to set up auditor", csv1alpha1.KubernetesAddon{}.APIVersion, csv1alpha1.KubernetesAddon{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&csv1alpha1.KubernetesAddon{}).
