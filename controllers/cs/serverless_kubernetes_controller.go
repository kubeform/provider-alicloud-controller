...
// ServerlessKubernetesReconciler reconciles a ServerlessKubernetes object
type ServerlessKubernetesReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cs.alicloud.kubeform.com,resources=serverlesskubernetes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cs.alicloud.kubeform.com,resources=serverlesskubernetes/status,verbs=get;update;patch

func (r *ServerlessKubernetesReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("serverlesskubernetes", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServerlessKubernetes")
...
func (r *ServerlessKubernetesReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &csv1alpha1.ServerlessKubernetes{}); err != nil {
			klog.Error(err, "unable to set up auditor", csv1alpha1.ServerlessKubernetes{}.APIVersion, csv1alpha1.ServerlessKubernetes{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&csv1alpha1.ServerlessKubernetes{}).
