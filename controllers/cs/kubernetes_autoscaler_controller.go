...
// KubernetesAutoscalerReconciler reconciles a KubernetesAutoscaler object
type KubernetesAutoscalerReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cs.alicloud.kubeform.com,resources=kubernetesautoscalers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cs.alicloud.kubeform.com,resources=kubernetesautoscalers/status,verbs=get;update;patch

func (r *KubernetesAutoscalerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("kubernetesautoscaler", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch KubernetesAutoscaler")
...
func (r *KubernetesAutoscalerReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &csv1alpha1.KubernetesAutoscaler{}); err != nil {
			klog.Error(err, "unable to set up auditor", csv1alpha1.KubernetesAutoscaler{}.APIVersion, csv1alpha1.KubernetesAutoscaler{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&csv1alpha1.KubernetesAutoscaler{}).
