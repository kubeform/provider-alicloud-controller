...
// EdgeKubernetesReconciler reconciles a EdgeKubernetes object
type EdgeKubernetesReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cs.alicloud.kubeform.com,resources=edgekubernetes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cs.alicloud.kubeform.com,resources=edgekubernetes/status,verbs=get;update;patch

func (r *EdgeKubernetesReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("edgekubernetes", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EdgeKubernetes")
...
func (r *EdgeKubernetesReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &csv1alpha1.EdgeKubernetes{}); err != nil {
			klog.Error(err, "unable to set up auditor", csv1alpha1.EdgeKubernetes{}.APIVersion, csv1alpha1.EdgeKubernetes{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&csv1alpha1.EdgeKubernetes{}).
...
func (r *EdgeKubernetesReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for EdgeKubernetes")
			return nil
		}
		...
		resourceList := &csv1alpha1.EdgeKubernetesList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list EdgeKubernetes")
			return nil
		}
