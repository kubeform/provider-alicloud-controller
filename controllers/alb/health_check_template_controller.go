...
// HealthCheckTemplateReconciler reconciles a HealthCheckTemplate object
type HealthCheckTemplateReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=alb.alicloud.kubeform.com,resources=healthchecktemplates,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=alb.alicloud.kubeform.com,resources=healthchecktemplates/status,verbs=get;update;patch

func (r *HealthCheckTemplateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("healthchecktemplate", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch HealthCheckTemplate")
...
func (r *HealthCheckTemplateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &albv1alpha1.HealthCheckTemplate{}); err != nil {
			klog.Error(err, "unable to set up auditor", albv1alpha1.HealthCheckTemplate{}.APIVersion, albv1alpha1.HealthCheckTemplate{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&albv1alpha1.HealthCheckTemplate{}).
