...
// EtlReconciler reconciles a Etl object
type EtlReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=log.alicloud.kubeform.com,resources=etls,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=log.alicloud.kubeform.com,resources=etls/status,verbs=get;update;patch

func (r *EtlReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("etl", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Etl")
...
func (r *EtlReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &logv1alpha1.Etl{}); err != nil {
			klog.Error(err, "unable to set up auditor", logv1alpha1.Etl{}.APIVersion, logv1alpha1.Etl{}.Kind)
			return err
		}
	}
...
func (r *EtlReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for Etl")
			return nil
		}
