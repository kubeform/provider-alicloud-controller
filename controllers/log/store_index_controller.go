...
// StoreIndexReconciler reconciles a StoreIndex object
type StoreIndexReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=log.alicloud.kubeform.com,resources=storeindices,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=log.alicloud.kubeform.com,resources=storeindices/status,verbs=get;update;patch

func (r *StoreIndexReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("storeindex", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch StoreIndex")
...
func (r *StoreIndexReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &logv1alpha1.StoreIndex{}); err != nil {
			klog.Error(err, "unable to set up auditor", logv1alpha1.StoreIndex{}.APIVersion, logv1alpha1.StoreIndex{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&logv1alpha1.StoreIndex{}).
