...
// +kubebuilder:rbac:groups=log.alicloud.kubeform.com,resources=machinegroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=log.alicloud.kubeform.com,resources=machinegroups/status,verbs=get;update;patch

func (r *MachineGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("machinegroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MachineGroup")
...
func (r *MachineGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &logv1alpha1.MachineGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", logv1alpha1.MachineGroup{}.APIVersion, logv1alpha1.MachineGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&logv1alpha1.MachineGroup{}).
