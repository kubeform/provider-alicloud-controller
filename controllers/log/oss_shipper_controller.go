...
// OssShipperReconciler reconciles a OssShipper object
type OssShipperReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=log.alicloud.kubeform.com,resources=ossshippers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=log.alicloud.kubeform.com,resources=ossshippers/status,verbs=get;update;patch

func (r *OssShipperReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ossshipper", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch OssShipper")
...
func (r *OssShipperReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &logv1alpha1.OssShipper{}); err != nil {
			klog.Error(err, "unable to set up auditor", logv1alpha1.OssShipper{}.APIVersion, logv1alpha1.OssShipper{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&logv1alpha1.OssShipper{}).
