...
// DomainConfigReconciler reconciles a DomainConfig object
type DomainConfigReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=scdn.alicloud.kubeform.com,resources=domainconfigs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=scdn.alicloud.kubeform.com,resources=domainconfigs/status,verbs=get;update;patch

func (r *DomainConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("domainconfig", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DomainConfig")
...
func (r *DomainConfigReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &scdnv1alpha1.DomainConfig{}); err != nil {
			klog.Error(err, "unable to set up auditor", scdnv1alpha1.DomainConfig{}.APIVersion, scdnv1alpha1.DomainConfig{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&scdnv1alpha1.DomainConfig{}).
