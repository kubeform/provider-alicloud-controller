...
// AccountPrivilegeReconciler reconciles a AccountPrivilege object
type AccountPrivilegeReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=polardb.alicloud.kubeform.com,resources=accountprivileges,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=polardb.alicloud.kubeform.com,resources=accountprivileges/status,verbs=get;update;patch

func (r *AccountPrivilegeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("accountprivilege", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AccountPrivilege")
...
func (r *AccountPrivilegeReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &polardbv1alpha1.AccountPrivilege{}); err != nil {
			klog.Error(err, "unable to set up auditor", polardbv1alpha1.AccountPrivilege{}.APIVersion, polardbv1alpha1.AccountPrivilege{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&polardbv1alpha1.AccountPrivilege{}).
