...
// EndpointAddressReconciler reconciles a EndpointAddress object
type EndpointAddressReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=polardb.alicloud.kubeform.com,resources=endpointaddresses,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=polardb.alicloud.kubeform.com,resources=endpointaddresses/status,verbs=get;update;patch

func (r *EndpointAddressReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("endpointaddress", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EndpointAddress")
...
func (r *EndpointAddressReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &polardbv1alpha1.EndpointAddress{}); err != nil {
			klog.Error(err, "unable to set up auditor", polardbv1alpha1.EndpointAddress{}.APIVersion, polardbv1alpha1.EndpointAddress{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&polardbv1alpha1.EndpointAddress{}).
