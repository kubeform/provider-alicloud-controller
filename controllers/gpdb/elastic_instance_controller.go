...
// ElasticInstanceReconciler reconciles a ElasticInstance object
type ElasticInstanceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=gpdb.alicloud.kubeform.com,resources=elasticinstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=gpdb.alicloud.kubeform.com,resources=elasticinstances/status,verbs=get;update;patch

func (r *ElasticInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("elasticinstance", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ElasticInstance")
...
func (r *ElasticInstanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &gpdbv1alpha1.ElasticInstance{}); err != nil {
			klog.Error(err, "unable to set up auditor", gpdbv1alpha1.ElasticInstance{}.APIVersion, gpdbv1alpha1.ElasticInstance{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&gpdbv1alpha1.ElasticInstance{}).
