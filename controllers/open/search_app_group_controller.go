...
// SearchAppGroupReconciler reconciles a SearchAppGroup object
type SearchAppGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=open.alicloud.kubeform.com,resources=searchappgroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=open.alicloud.kubeform.com,resources=searchappgroups/status,verbs=get;update;patch

func (r *SearchAppGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("searchappgroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SearchAppGroup")
...
func (r *SearchAppGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &openv1alpha1.SearchAppGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", openv1alpha1.SearchAppGroup{}.APIVersion, openv1alpha1.SearchAppGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&openv1alpha1.SearchAppGroup{}).
