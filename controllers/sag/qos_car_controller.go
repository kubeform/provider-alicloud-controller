...
// QosCarReconciler reconciles a QosCar object
type QosCarReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=sag.alicloud.kubeform.com,resources=qoscars,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sag.alicloud.kubeform.com,resources=qoscars/status,verbs=get;update;patch

func (r *QosCarReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("qoscar", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch QosCar")
...
func (r *QosCarReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sagv1alpha1.QosCar{}); err != nil {
			klog.Error(err, "unable to set up auditor", sagv1alpha1.QosCar{}.APIVersion, sagv1alpha1.QosCar{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sagv1alpha1.QosCar{}).
