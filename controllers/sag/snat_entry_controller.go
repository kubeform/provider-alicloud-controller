...
// SnatEntryReconciler reconciles a SnatEntry object
type SnatEntryReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=sag.alicloud.kubeform.com,resources=snatentries,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sag.alicloud.kubeform.com,resources=snatentries/status,verbs=get;update;patch

func (r *SnatEntryReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("snatentry", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SnatEntry")
...
func (r *SnatEntryReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sagv1alpha1.SnatEntry{}); err != nil {
			klog.Error(err, "unable to set up auditor", sagv1alpha1.SnatEntry{}.APIVersion, sagv1alpha1.SnatEntry{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sagv1alpha1.SnatEntry{}).
