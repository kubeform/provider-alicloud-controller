...
// DnatEntryReconciler reconciles a DnatEntry object
type DnatEntryReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=sag.alicloud.kubeform.com,resources=dnatentries,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sag.alicloud.kubeform.com,resources=dnatentries/status,verbs=get;update;patch

func (r *DnatEntryReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dnatentry", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DnatEntry")
...
func (r *DnatEntryReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sagv1alpha1.DnatEntry{}); err != nil {
			klog.Error(err, "unable to set up auditor", sagv1alpha1.DnatEntry{}.APIVersion, sagv1alpha1.DnatEntry{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sagv1alpha1.DnatEntry{}).
