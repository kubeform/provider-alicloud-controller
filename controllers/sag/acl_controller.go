...
// +kubebuilder:rbac:groups=sag.alicloud.kubeform.com,resources=acls,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sag.alicloud.kubeform.com,resources=acls/status,verbs=get;update;patch

func (r *AclReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("acl", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Acl")
...
func (r *AclReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sagv1alpha1.Acl{}); err != nil {
			klog.Error(err, "unable to set up auditor", sagv1alpha1.Acl{}.APIVersion, sagv1alpha1.Acl{}.Kind)
			return err
		}
	}
