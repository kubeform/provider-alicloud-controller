...
// QosPolicyReconciler reconciles a QosPolicy object
...
// +kubebuilder:rbac:groups=sag.alicloud.kubeform.com,resources=qospolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sag.alicloud.kubeform.com,resources=qospolicies/status,verbs=get;update;patch

func (r *QosPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("qospolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch QosPolicy")
...
func (r *QosPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sagv1alpha1.QosPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", sagv1alpha1.QosPolicy{}.APIVersion, sagv1alpha1.QosPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sagv1alpha1.QosPolicy{}).
