...
// ManagerControlPolicyAttachmentReconciler reconciles a ManagerControlPolicyAttachment object
type ManagerControlPolicyAttachmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=resource.alicloud.kubeform.com,resources=managercontrolpolicyattachments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=resource.alicloud.kubeform.com,resources=managercontrolpolicyattachments/status,verbs=get;update;patch

func (r *ManagerControlPolicyAttachmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("managercontrolpolicyattachment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ManagerControlPolicyAttachment")
...
func (r *ManagerControlPolicyAttachmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &resourcev1alpha1.ManagerControlPolicyAttachment{}); err != nil {
			klog.Error(err, "unable to set up auditor", resourcev1alpha1.ManagerControlPolicyAttachment{}.APIVersion, resourcev1alpha1.ManagerControlPolicyAttachment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&resourcev1alpha1.ManagerControlPolicyAttachment{}).
