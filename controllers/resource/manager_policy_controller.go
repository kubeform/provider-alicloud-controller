...
// ManagerPolicyReconciler reconciles a ManagerPolicy object
type ManagerPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=resource.alicloud.kubeform.com,resources=managerpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=resource.alicloud.kubeform.com,resources=managerpolicies/status,verbs=get;update;patch

func (r *ManagerPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("managerpolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ManagerPolicy")
...
func (r *ManagerPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &resourcev1alpha1.ManagerPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", resourcev1alpha1.ManagerPolicy{}.APIVersion, resourcev1alpha1.ManagerPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&resourcev1alpha1.ManagerPolicy{}).
