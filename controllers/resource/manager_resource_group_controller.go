...
// ManagerResourceGroupReconciler reconciles a ManagerResourceGroup object
type ManagerResourceGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=resource.alicloud.kubeform.com,resources=managerresourcegroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=resource.alicloud.kubeform.com,resources=managerresourcegroups/status,verbs=get;update;patch

func (r *ManagerResourceGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("managerresourcegroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ManagerResourceGroup")
...
func (r *ManagerResourceGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &resourcev1alpha1.ManagerResourceGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", resourcev1alpha1.ManagerResourceGroup{}.APIVersion, resourcev1alpha1.ManagerResourceGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&resourcev1alpha1.ManagerResourceGroup{}).
