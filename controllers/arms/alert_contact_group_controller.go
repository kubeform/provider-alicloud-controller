...
// AlertContactGroupReconciler reconciles a AlertContactGroup object
type AlertContactGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=arms.alicloud.kubeform.com,resources=alertcontactgroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=arms.alicloud.kubeform.com,resources=alertcontactgroups/status,verbs=get;update;patch

func (r *AlertContactGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("alertcontactgroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AlertContactGroup")
...
func (r *AlertContactGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &armsv1alpha1.AlertContactGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", armsv1alpha1.AlertContactGroup{}.APIVersion, armsv1alpha1.AlertContactGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&armsv1alpha1.AlertContactGroup{}).
