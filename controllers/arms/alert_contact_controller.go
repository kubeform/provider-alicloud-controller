...
// +kubebuilder:rbac:groups=arms.alicloud.kubeform.com,resources=alertcontacts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=arms.alicloud.kubeform.com,resources=alertcontacts/status,verbs=get;update;patch

func (r *AlertContactReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("alertcontact", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AlertContact")
...
func (r *AlertContactReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &armsv1alpha1.AlertContact{}); err != nil {
			klog.Error(err, "unable to set up auditor", armsv1alpha1.AlertContact{}.APIVersion, armsv1alpha1.AlertContact{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&armsv1alpha1.AlertContact{}).
