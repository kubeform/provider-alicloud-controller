...
// PrometheusAlertRuleReconciler reconciles a PrometheusAlertRule object
type PrometheusAlertRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=arms.alicloud.kubeform.com,resources=prometheusalertrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=arms.alicloud.kubeform.com,resources=prometheusalertrules/status,verbs=get;update;patch

func (r *PrometheusAlertRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("prometheusalertrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PrometheusAlertRule")
...
func (r *PrometheusAlertRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &armsv1alpha1.PrometheusAlertRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", armsv1alpha1.PrometheusAlertRule{}.APIVersion, armsv1alpha1.PrometheusAlertRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&armsv1alpha1.PrometheusAlertRule{}).
