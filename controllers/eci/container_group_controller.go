...
// ContainerGroupReconciler reconciles a ContainerGroup object
type ContainerGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=eci.alicloud.kubeform.com,resources=containergroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=eci.alicloud.kubeform.com,resources=containergroups/status,verbs=get;update;patch

func (r *ContainerGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("containergroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ContainerGroup")
...
func (r *ContainerGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &eciv1alpha1.ContainerGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", eciv1alpha1.ContainerGroup{}.APIVersion, eciv1alpha1.ContainerGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&eciv1alpha1.ContainerGroup{}).
