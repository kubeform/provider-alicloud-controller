...
// ImageCacheReconciler reconciles a ImageCache object
...
// +kubebuilder:rbac:groups=eci.alicloud.kubeform.com,resources=imagecaches,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=eci.alicloud.kubeform.com,resources=imagecaches/status,verbs=get;update;patch

func (r *ImageCacheReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("imagecache", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ImageCache")
...
func (r *ImageCacheReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &eciv1alpha1.ImageCache{}); err != nil {
			klog.Error(err, "unable to set up auditor", eciv1alpha1.ImageCache{}.APIVersion, eciv1alpha1.ImageCache{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&eciv1alpha1.ImageCache{}).
