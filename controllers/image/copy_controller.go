...
// +kubebuilder:rbac:groups=image.alicloud.kubeform.com,resources=copies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=image.alicloud.kubeform.com,resources=copies/status,verbs=get;update;patch

func (r *CopyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("copy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Copy")
...
func (r *CopyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &imagev1alpha1.Copy{}); err != nil {
			klog.Error(err, "unable to set up auditor", imagev1alpha1.Copy{}.APIVersion, imagev1alpha1.Copy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&imagev1alpha1.Copy{}).
