...
// SharePermissionReconciler reconciles a SharePermission object
type SharePermissionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=image.alicloud.kubeform.com,resources=sharepermissions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=image.alicloud.kubeform.com,resources=sharepermissions/status,verbs=get;update;patch

func (r *SharePermissionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("sharepermission", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SharePermission")
...
func (r *SharePermissionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &imagev1alpha1.SharePermission{}); err != nil {
			klog.Error(err, "unable to set up auditor", imagev1alpha1.SharePermission{}.APIVersion, imagev1alpha1.SharePermission{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&imagev1alpha1.SharePermission{}).
