...
// +kubebuilder:rbac:groups=image.alicloud.kubeform.com,resources=imports,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=image.alicloud.kubeform.com,resources=imports/status,verbs=get;update;patch
...
func (r *ImportReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Import")
...
func (r *ImportReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &imagev1alpha1.Import{}); err != nil {
			klog.Error(err, "unable to set up auditor", imagev1alpha1.Import{}.APIVersion, imagev1alpha1.Import{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&imagev1alpha1.Import{}).
