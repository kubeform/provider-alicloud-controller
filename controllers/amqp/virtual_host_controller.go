...
// +kubebuilder:rbac:groups=amqp.alicloud.kubeform.com,resources=virtualhosts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=amqp.alicloud.kubeform.com,resources=virtualhosts/status,verbs=get;update;patch

func (r *VirtualHostReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("virtualhost", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VirtualHost")
...
func (r *VirtualHostReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &amqpv1alpha1.VirtualHost{}); err != nil {
			klog.Error(err, "unable to set up auditor", amqpv1alpha1.VirtualHost{}.APIVersion, amqpv1alpha1.VirtualHost{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&amqpv1alpha1.VirtualHost{}).
