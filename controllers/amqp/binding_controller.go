...
// +kubebuilder:rbac:groups=amqp.alicloud.kubeform.com,resources=bindings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=amqp.alicloud.kubeform.com,resources=bindings/status,verbs=get;update;patch
...
func (r *BindingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Binding")
...
func (r *BindingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &amqpv1alpha1.Binding{}); err != nil {
			klog.Error(err, "unable to set up auditor", amqpv1alpha1.Binding{}.APIVersion, amqpv1alpha1.Binding{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&amqpv1alpha1.Binding{}).
