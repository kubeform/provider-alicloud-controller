...
// WorksFolderReconciler reconciles a WorksFolder object
type WorksFolderReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=data.alicloud.kubeform.com,resources=worksfolders,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=data.alicloud.kubeform.com,resources=worksfolders/status,verbs=get;update;patch

func (r *WorksFolderReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("worksfolder", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch WorksFolder")
...
func (r *WorksFolderReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datav1alpha1.WorksFolder{}); err != nil {
			klog.Error(err, "unable to set up auditor", datav1alpha1.WorksFolder{}.APIVersion, datav1alpha1.WorksFolder{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datav1alpha1.WorksFolder{}).
