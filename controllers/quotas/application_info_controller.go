...
// ApplicationInfoReconciler reconciles a ApplicationInfo object
type ApplicationInfoReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=quotas.alicloud.kubeform.com,resources=applicationinfoes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=quotas.alicloud.kubeform.com,resources=applicationinfoes/status,verbs=get;update;patch

func (r *ApplicationInfoReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("applicationinfo", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ApplicationInfo")
...
func (r *ApplicationInfoReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &quotasv1alpha1.ApplicationInfo{}); err != nil {
			klog.Error(err, "unable to set up auditor", quotasv1alpha1.ApplicationInfo{}.APIVersion, quotasv1alpha1.ApplicationInfo{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&quotasv1alpha1.ApplicationInfo{}).
