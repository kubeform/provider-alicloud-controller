...
// QuotaAlarmReconciler reconciles a QuotaAlarm object
type QuotaAlarmReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=quotas.alicloud.kubeform.com,resources=quotaalarms,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=quotas.alicloud.kubeform.com,resources=quotaalarms/status,verbs=get;update;patch

func (r *QuotaAlarmReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("quotaalarm", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch QuotaAlarm")
...
func (r *QuotaAlarmReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &quotasv1alpha1.QuotaAlarm{}); err != nil {
			klog.Error(err, "unable to set up auditor", quotasv1alpha1.QuotaAlarm{}.APIVersion, quotasv1alpha1.QuotaAlarm{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&quotasv1alpha1.QuotaAlarm{}).
