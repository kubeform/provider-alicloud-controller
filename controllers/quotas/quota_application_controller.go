...
// QuotaApplicationReconciler reconciles a QuotaApplication object
type QuotaApplicationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=quotas.alicloud.kubeform.com,resources=quotaapplications,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=quotas.alicloud.kubeform.com,resources=quotaapplications/status,verbs=get;update;patch

func (r *QuotaApplicationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("quotaapplication", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch QuotaApplication")
...
func (r *QuotaApplicationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &quotasv1alpha1.QuotaApplication{}); err != nil {
			klog.Error(err, "unable to set up auditor", quotasv1alpha1.QuotaApplication{}.APIVersion, quotasv1alpha1.QuotaApplication{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&quotasv1alpha1.QuotaApplication{}).
