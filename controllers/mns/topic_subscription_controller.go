...
// TopicSubscriptionReconciler reconciles a TopicSubscription object
type TopicSubscriptionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=mns.alicloud.kubeform.com,resources=topicsubscriptions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=mns.alicloud.kubeform.com,resources=topicsubscriptions/status,verbs=get;update;patch

func (r *TopicSubscriptionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("topicsubscription", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TopicSubscription")
...
func (r *TopicSubscriptionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &mnsv1alpha1.TopicSubscription{}); err != nil {
			klog.Error(err, "unable to set up auditor", mnsv1alpha1.TopicSubscription{}.APIVersion, mnsv1alpha1.TopicSubscription{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&mnsv1alpha1.TopicSubscription{}).
