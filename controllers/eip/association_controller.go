...
// +kubebuilder:rbac:groups=eip.alicloud.kubeform.com,resources=associations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=eip.alicloud.kubeform.com,resources=associations/status,verbs=get;update;patch

func (r *AssociationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("association", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Association")
...
func (r *AssociationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &eipv1alpha1.Association{}); err != nil {
			klog.Error(err, "unable to set up auditor", eipv1alpha1.Association{}.APIVersion, eipv1alpha1.Association{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&eipv1alpha1.Association{}).
