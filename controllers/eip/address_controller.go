...
// +kubebuilder:rbac:groups=eip.alicloud.kubeform.com,resources=addresses,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=eip.alicloud.kubeform.com,resources=addresses/status,verbs=get;update;patch

func (r *AddressReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("address", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Address")
...
func (r *AddressReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &eipv1alpha1.Address{}); err != nil {
			klog.Error(err, "unable to set up auditor", eipv1alpha1.Address{}.APIVersion, eipv1alpha1.Address{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&eipv1alpha1.Address{}).
