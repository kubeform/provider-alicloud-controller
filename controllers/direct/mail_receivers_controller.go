...
// MailReceiversReconciler reconciles a MailReceivers object
type MailReceiversReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=direct.alicloud.kubeform.com,resources=mailreceivers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=direct.alicloud.kubeform.com,resources=mailreceivers/status,verbs=get;update;patch

func (r *MailReceiversReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("mailreceivers", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MailReceivers")
...
func (r *MailReceiversReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &directv1alpha1.MailReceivers{}); err != nil {
			klog.Error(err, "unable to set up auditor", directv1alpha1.MailReceivers{}.APIVersion, directv1alpha1.MailReceivers{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&directv1alpha1.MailReceivers{}).
