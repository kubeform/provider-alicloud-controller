...
// MailTagReconciler reconciles a MailTag object
type MailTagReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=direct.alicloud.kubeform.com,resources=mailtags,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=direct.alicloud.kubeform.com,resources=mailtags/status,verbs=get;update;patch

func (r *MailTagReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("mailtag", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MailTag")
...
func (r *MailTagReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &directv1alpha1.MailTag{}); err != nil {
			klog.Error(err, "unable to set up auditor", directv1alpha1.MailTag{}.APIVersion, directv1alpha1.MailTag{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&directv1alpha1.MailTag{}).
