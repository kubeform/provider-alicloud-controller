...
// MailDomainReconciler reconciles a MailDomain object
type MailDomainReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=direct.alicloud.kubeform.com,resources=maildomains,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=direct.alicloud.kubeform.com,resources=maildomains/status,verbs=get;update;patch

func (r *MailDomainReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("maildomain", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MailDomain")
...
func (r *MailDomainReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &directv1alpha1.MailDomain{}); err != nil {
			klog.Error(err, "unable to set up auditor", directv1alpha1.MailDomain{}.APIVersion, directv1alpha1.MailDomain{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&directv1alpha1.MailDomain{}).
