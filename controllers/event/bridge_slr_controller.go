...
// BridgeSlrReconciler reconciles a BridgeSlr object
type BridgeSlrReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=event.alicloud.kubeform.com,resources=bridgeslrs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=event.alicloud.kubeform.com,resources=bridgeslrs/status,verbs=get;update;patch

func (r *BridgeSlrReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("bridgeslr", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BridgeSlr")
...
func (r *BridgeSlrReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &eventv1alpha1.BridgeSlr{}); err != nil {
			klog.Error(err, "unable to set up auditor", eventv1alpha1.BridgeSlr{}.APIVersion, eventv1alpha1.BridgeSlr{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&eventv1alpha1.BridgeSlr{}).
