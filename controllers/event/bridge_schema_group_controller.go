...
// BridgeSchemaGroupReconciler reconciles a BridgeSchemaGroup object
type BridgeSchemaGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=event.alicloud.kubeform.com,resources=bridgeschemagroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=event.alicloud.kubeform.com,resources=bridgeschemagroups/status,verbs=get;update;patch

func (r *BridgeSchemaGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("bridgeschemagroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BridgeSchemaGroup")
...
func (r *BridgeSchemaGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &eventv1alpha1.BridgeSchemaGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", eventv1alpha1.BridgeSchemaGroup{}.APIVersion, eventv1alpha1.BridgeSchemaGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&eventv1alpha1.BridgeSchemaGroup{}).
