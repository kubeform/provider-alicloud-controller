...
// BridgeEventSourceReconciler reconciles a BridgeEventSource object
type BridgeEventSourceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=event.alicloud.kubeform.com,resources=bridgeeventsources,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=event.alicloud.kubeform.com,resources=bridgeeventsources/status,verbs=get;update;patch

func (r *BridgeEventSourceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("bridgeeventsource", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BridgeEventSource")
...
func (r *BridgeEventSourceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &eventv1alpha1.BridgeEventSource{}); err != nil {
			klog.Error(err, "unable to set up auditor", eventv1alpha1.BridgeEventSource{}.APIVersion, eventv1alpha1.BridgeEventSource{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&eventv1alpha1.BridgeEventSource{}).
