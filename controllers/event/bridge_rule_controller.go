...
// BridgeRuleReconciler reconciles a BridgeRule object
type BridgeRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=event.alicloud.kubeform.com,resources=bridgerules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=event.alicloud.kubeform.com,resources=bridgerules/status,verbs=get;update;patch

func (r *BridgeRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("bridgerule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BridgeRule")
...
func (r *BridgeRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &eventv1alpha1.BridgeRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", eventv1alpha1.BridgeRule{}.APIVersion, eventv1alpha1.BridgeRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&eventv1alpha1.BridgeRule{}).
