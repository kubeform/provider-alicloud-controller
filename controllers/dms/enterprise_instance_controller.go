...
// EnterpriseInstanceReconciler reconciles a EnterpriseInstance object
type EnterpriseInstanceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dms.alicloud.kubeform.com,resources=enterpriseinstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dms.alicloud.kubeform.com,resources=enterpriseinstances/status,verbs=get;update;patch

func (r *EnterpriseInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("enterpriseinstance", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EnterpriseInstance")
...
func (r *EnterpriseInstanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dmsv1alpha1.EnterpriseInstance{}); err != nil {
			klog.Error(err, "unable to set up auditor", dmsv1alpha1.EnterpriseInstance{}.APIVersion, dmsv1alpha1.EnterpriseInstance{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dmsv1alpha1.EnterpriseInstance{}).
...
func (r *EnterpriseInstanceReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for EnterpriseInstance")
			return nil
		}
		...
		resourceList := &dmsv1alpha1.EnterpriseInstanceList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list EnterpriseInstance")
			return nil
		}
