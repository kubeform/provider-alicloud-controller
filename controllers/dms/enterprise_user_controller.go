...
// EnterpriseUserReconciler reconciles a EnterpriseUser object
type EnterpriseUserReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dms.alicloud.kubeform.com,resources=enterpriseusers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dms.alicloud.kubeform.com,resources=enterpriseusers/status,verbs=get;update;patch

func (r *EnterpriseUserReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("enterpriseuser", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EnterpriseUser")
...
func (r *EnterpriseUserReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dmsv1alpha1.EnterpriseUser{}); err != nil {
			klog.Error(err, "unable to set up auditor", dmsv1alpha1.EnterpriseUser{}.APIVersion, dmsv1alpha1.EnterpriseUser{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dmsv1alpha1.EnterpriseUser{}).
