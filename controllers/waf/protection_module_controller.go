...
// ProtectionModuleReconciler reconciles a ProtectionModule object
type ProtectionModuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=waf.alicloud.kubeform.com,resources=protectionmodules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=waf.alicloud.kubeform.com,resources=protectionmodules/status,verbs=get;update;patch

func (r *ProtectionModuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("protectionmodule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ProtectionModule")
...
func (r *ProtectionModuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &wafv1alpha1.ProtectionModule{}); err != nil {
			klog.Error(err, "unable to set up auditor", wafv1alpha1.ProtectionModule{}.APIVersion, wafv1alpha1.ProtectionModule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&wafv1alpha1.ProtectionModule{}).
