...
// DbauditInstanceReconciler reconciles a DbauditInstance object
type DbauditInstanceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=yundun.alicloud.kubeform.com,resources=dbauditinstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=yundun.alicloud.kubeform.com,resources=dbauditinstances/status,verbs=get;update;patch

func (r *DbauditInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dbauditinstance", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DbauditInstance")
...
func (r *DbauditInstanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &yundunv1alpha1.DbauditInstance{}); err != nil {
			klog.Error(err, "unable to set up auditor", yundunv1alpha1.DbauditInstance{}.APIVersion, yundunv1alpha1.DbauditInstance{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&yundunv1alpha1.DbauditInstance{}).
