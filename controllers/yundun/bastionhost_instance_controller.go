...
// BastionhostInstanceReconciler reconciles a BastionhostInstance object
type BastionhostInstanceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=yundun.alicloud.kubeform.com,resources=bastionhostinstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=yundun.alicloud.kubeform.com,resources=bastionhostinstances/status,verbs=get;update;patch

func (r *BastionhostInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("bastionhostinstance", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BastionhostInstance")
...
func (r *BastionhostInstanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &yundunv1alpha1.BastionhostInstance{}); err != nil {
			klog.Error(err, "unable to set up auditor", yundunv1alpha1.BastionhostInstance{}.APIVersion, yundunv1alpha1.BastionhostInstance{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&yundunv1alpha1.BastionhostInstance{}).
