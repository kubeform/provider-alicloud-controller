...
// NetworkInterfaceAttachmentReconciler reconciles a NetworkInterfaceAttachment object
type NetworkInterfaceAttachmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ecs.alicloud.kubeform.com,resources=networkinterfaceattachments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ecs.alicloud.kubeform.com,resources=networkinterfaceattachments/status,verbs=get;update;patch

func (r *NetworkInterfaceAttachmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("networkinterfaceattachment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NetworkInterfaceAttachment")
...
func (r *NetworkInterfaceAttachmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ecsv1alpha1.NetworkInterfaceAttachment{}); err != nil {
			klog.Error(err, "unable to set up auditor", ecsv1alpha1.NetworkInterfaceAttachment{}.APIVersion, ecsv1alpha1.NetworkInterfaceAttachment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ecsv1alpha1.NetworkInterfaceAttachment{}).
