...
// LaunchTemplateReconciler reconciles a LaunchTemplate object
...
// +kubebuilder:rbac:groups=ecs.alicloud.kubeform.com,resources=launchtemplates,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ecs.alicloud.kubeform.com,resources=launchtemplates/status,verbs=get;update;patch

func (r *LaunchTemplateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("launchtemplate", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LaunchTemplate")
...
func (r *LaunchTemplateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ecsv1alpha1.LaunchTemplate{}); err != nil {
			klog.Error(err, "unable to set up auditor", ecsv1alpha1.LaunchTemplate{}.APIVersion, ecsv1alpha1.LaunchTemplate{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ecsv1alpha1.LaunchTemplate{}).
