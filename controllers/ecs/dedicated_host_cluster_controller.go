...
// DedicatedHostClusterReconciler reconciles a DedicatedHostCluster object
type DedicatedHostClusterReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ecs.alicloud.kubeform.com,resources=dedicatedhostclusters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ecs.alicloud.kubeform.com,resources=dedicatedhostclusters/status,verbs=get;update;patch

func (r *DedicatedHostClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dedicatedhostcluster", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DedicatedHostCluster")
...
func (r *DedicatedHostClusterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ecsv1alpha1.DedicatedHostCluster{}); err != nil {
			klog.Error(err, "unable to set up auditor", ecsv1alpha1.DedicatedHostCluster{}.APIVersion, ecsv1alpha1.DedicatedHostCluster{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ecsv1alpha1.DedicatedHostCluster{}).
