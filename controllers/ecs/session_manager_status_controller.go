...
// SessionManagerStatusReconciler reconciles a SessionManagerStatus object
type SessionManagerStatusReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ecs.alicloud.kubeform.com,resources=sessionmanagerstatuses,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ecs.alicloud.kubeform.com,resources=sessionmanagerstatuses/status,verbs=get;update;patch

func (r *SessionManagerStatusReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("sessionmanagerstatus", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SessionManagerStatus")
...
func (r *SessionManagerStatusReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ecsv1alpha1.SessionManagerStatus{}); err != nil {
			klog.Error(err, "unable to set up auditor", ecsv1alpha1.SessionManagerStatus{}.APIVersion, ecsv1alpha1.SessionManagerStatus{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ecsv1alpha1.SessionManagerStatus{}).
