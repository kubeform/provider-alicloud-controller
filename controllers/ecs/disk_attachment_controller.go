...
// +kubebuilder:rbac:groups=ecs.alicloud.kubeform.com,resources=diskattachments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ecs.alicloud.kubeform.com,resources=diskattachments/status,verbs=get;update;patch

func (r *DiskAttachmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("diskattachment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DiskAttachment")
...
func (r *DiskAttachmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ecsv1alpha1.DiskAttachment{}); err != nil {
			klog.Error(err, "unable to set up auditor", ecsv1alpha1.DiskAttachment{}.APIVersion, ecsv1alpha1.DiskAttachment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ecsv1alpha1.DiskAttachment{}).
