...
// +kubebuilder:rbac:groups=ecs.alicloud.kubeform.com,resources=networkinterfaces,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ecs.alicloud.kubeform.com,resources=networkinterfaces/status,verbs=get;update;patch
...
func (r *NetworkInterfaceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NetworkInterface")
...
func (r *NetworkInterfaceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ecsv1alpha1.NetworkInterface{}); err != nil {
			klog.Error(err, "unable to set up auditor", ecsv1alpha1.NetworkInterface{}.APIVersion, ecsv1alpha1.NetworkInterface{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ecsv1alpha1.NetworkInterface{}).
