...
// HouseDbClusterReconciler reconciles a HouseDbCluster object
type HouseDbClusterReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=click.alicloud.kubeform.com,resources=housedbclusters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=click.alicloud.kubeform.com,resources=housedbclusters/status,verbs=get;update;patch

func (r *HouseDbClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("housedbcluster", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch HouseDbCluster")
...
func (r *HouseDbClusterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &clickv1alpha1.HouseDbCluster{}); err != nil {
			klog.Error(err, "unable to set up auditor", clickv1alpha1.HouseDbCluster{}.APIVersion, clickv1alpha1.HouseDbCluster{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&clickv1alpha1.HouseDbCluster{}).
