...
// HouseAccountReconciler reconciles a HouseAccount object
type HouseAccountReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=click.alicloud.kubeform.com,resources=houseaccounts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=click.alicloud.kubeform.com,resources=houseaccounts/status,verbs=get;update;patch

func (r *HouseAccountReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("houseaccount", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch HouseAccount")
...
func (r *HouseAccountReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &clickv1alpha1.HouseAccount{}); err != nil {
			klog.Error(err, "unable to set up auditor", clickv1alpha1.HouseAccount{}.APIVersion, clickv1alpha1.HouseAccount{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&clickv1alpha1.HouseAccount{}).
