...
// +kubebuilder:rbac:groups=vpc.alicloud.kubeform.com,resources=bgppeers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vpc.alicloud.kubeform.com,resources=bgppeers/status,verbs=get;update;patch
...
func (r *BgpPeerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BgpPeer")
...
func (r *BgpPeerReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vpcv1alpha1.BgpPeer{}); err != nil {
			klog.Error(err, "unable to set up auditor", vpcv1alpha1.BgpPeer{}.APIVersion, vpcv1alpha1.BgpPeer{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vpcv1alpha1.BgpPeer{}).
