...
// Ipv6EgressRuleReconciler reconciles a Ipv6EgressRule object
type Ipv6EgressRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=vpc.alicloud.kubeform.com,resources=ipv6egressrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vpc.alicloud.kubeform.com,resources=ipv6egressrules/status,verbs=get;update;patch

func (r *Ipv6EgressRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ipv6egressrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Ipv6EgressRule")
...
func (r *Ipv6EgressRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vpcv1alpha1.Ipv6EgressRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", vpcv1alpha1.Ipv6EgressRule{}.APIVersion, vpcv1alpha1.Ipv6EgressRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vpcv1alpha1.Ipv6EgressRule{}).
