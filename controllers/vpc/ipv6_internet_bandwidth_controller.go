...
// Ipv6InternetBandwidthReconciler reconciles a Ipv6InternetBandwidth object
type Ipv6InternetBandwidthReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=vpc.alicloud.kubeform.com,resources=ipv6internetbandwidths,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vpc.alicloud.kubeform.com,resources=ipv6internetbandwidths/status,verbs=get;update;patch

func (r *Ipv6InternetBandwidthReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ipv6internetbandwidth", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Ipv6InternetBandwidth")
...
func (r *Ipv6InternetBandwidthReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vpcv1alpha1.Ipv6InternetBandwidth{}); err != nil {
			klog.Error(err, "unable to set up auditor", vpcv1alpha1.Ipv6InternetBandwidth{}.APIVersion, vpcv1alpha1.Ipv6InternetBandwidth{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vpcv1alpha1.Ipv6InternetBandwidth{}).
