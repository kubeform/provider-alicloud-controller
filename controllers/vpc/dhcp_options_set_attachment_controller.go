...
// DhcpOptionsSetAttachmentReconciler reconciles a DhcpOptionsSetAttachment object
type DhcpOptionsSetAttachmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=vpc.alicloud.kubeform.com,resources=dhcpoptionssetattachments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vpc.alicloud.kubeform.com,resources=dhcpoptionssetattachments/status,verbs=get;update;patch

func (r *DhcpOptionsSetAttachmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dhcpoptionssetattachment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DhcpOptionsSetAttachment")
...
func (r *DhcpOptionsSetAttachmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vpcv1alpha1.DhcpOptionsSetAttachment{}); err != nil {
			klog.Error(err, "unable to set up auditor", vpcv1alpha1.DhcpOptionsSetAttachment{}.APIVersion, vpcv1alpha1.DhcpOptionsSetAttachment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vpcv1alpha1.DhcpOptionsSetAttachment{}).
