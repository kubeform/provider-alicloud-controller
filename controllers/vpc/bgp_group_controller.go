...
// BgpGroupReconciler reconciles a BgpGroup object
type BgpGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=vpc.alicloud.kubeform.com,resources=bgpgroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vpc.alicloud.kubeform.com,resources=bgpgroups/status,verbs=get;update;patch

func (r *BgpGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("bgpgroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BgpGroup")
...
func (r *BgpGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vpcv1alpha1.BgpGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", vpcv1alpha1.BgpGroup{}.APIVersion, vpcv1alpha1.BgpGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vpcv1alpha1.BgpGroup{}).
