...
// TrafficMirrorFilterEgressRuleReconciler reconciles a TrafficMirrorFilterEgressRule object
type TrafficMirrorFilterEgressRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=vpc.alicloud.kubeform.com,resources=trafficmirrorfilteregressrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vpc.alicloud.kubeform.com,resources=trafficmirrorfilteregressrules/status,verbs=get;update;patch

func (r *TrafficMirrorFilterEgressRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("trafficmirrorfilteregressrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TrafficMirrorFilterEgressRule")
...
func (r *TrafficMirrorFilterEgressRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vpcv1alpha1.TrafficMirrorFilterEgressRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", vpcv1alpha1.TrafficMirrorFilterEgressRule{}.APIVersion, vpcv1alpha1.TrafficMirrorFilterEgressRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vpcv1alpha1.TrafficMirrorFilterEgressRule{}).
