...
// TrafficMirrorFilterIngressRuleReconciler reconciles a TrafficMirrorFilterIngressRule object
type TrafficMirrorFilterIngressRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=vpc.alicloud.kubeform.com,resources=trafficmirrorfilteringressrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vpc.alicloud.kubeform.com,resources=trafficmirrorfilteringressrules/status,verbs=get;update;patch

func (r *TrafficMirrorFilterIngressRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("trafficmirrorfilteringressrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TrafficMirrorFilterIngressRule")
...
func (r *TrafficMirrorFilterIngressRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vpcv1alpha1.TrafficMirrorFilterIngressRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", vpcv1alpha1.TrafficMirrorFilterIngressRule{}.APIVersion, vpcv1alpha1.TrafficMirrorFilterIngressRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vpcv1alpha1.TrafficMirrorFilterIngressRule{}).
