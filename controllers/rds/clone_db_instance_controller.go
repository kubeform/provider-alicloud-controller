...
// CloneDbInstanceReconciler reconciles a CloneDbInstance object
type CloneDbInstanceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=rds.alicloud.kubeform.com,resources=clonedbinstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=rds.alicloud.kubeform.com,resources=clonedbinstances/status,verbs=get;update;patch

func (r *CloneDbInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("clonedbinstance", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CloneDbInstance")
...
func (r *CloneDbInstanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &rdsv1alpha1.CloneDbInstance{}); err != nil {
			klog.Error(err, "unable to set up auditor", rdsv1alpha1.CloneDbInstance{}.APIVersion, rdsv1alpha1.CloneDbInstance{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&rdsv1alpha1.CloneDbInstance{}).
