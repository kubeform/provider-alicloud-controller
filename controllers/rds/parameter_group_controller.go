...
// ParameterGroupReconciler reconciles a ParameterGroup object
type ParameterGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=rds.alicloud.kubeform.com,resources=parametergroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=rds.alicloud.kubeform.com,resources=parametergroups/status,verbs=get;update;patch

func (r *ParameterGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("parametergroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ParameterGroup")
...
func (r *ParameterGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &rdsv1alpha1.ParameterGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", rdsv1alpha1.ParameterGroup{}.APIVersion, rdsv1alpha1.ParameterGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&rdsv1alpha1.ParameterGroup{}).
