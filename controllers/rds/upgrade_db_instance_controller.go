...
// UpgradeDbInstanceReconciler reconciles a UpgradeDbInstance object
type UpgradeDbInstanceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=rds.alicloud.kubeform.com,resources=upgradedbinstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=rds.alicloud.kubeform.com,resources=upgradedbinstances/status,verbs=get;update;patch

func (r *UpgradeDbInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("upgradedbinstance", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch UpgradeDbInstance")
...
func (r *UpgradeDbInstanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &rdsv1alpha1.UpgradeDbInstance{}); err != nil {
			klog.Error(err, "unable to set up auditor", rdsv1alpha1.UpgradeDbInstance{}.APIVersion, rdsv1alpha1.UpgradeDbInstance{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&rdsv1alpha1.UpgradeDbInstance{}).
