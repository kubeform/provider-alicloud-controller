...
// DeliveryChannelReconciler reconciles a DeliveryChannel object
type DeliveryChannelReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=config.alicloud.kubeform.com,resources=deliverychannels,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=config.alicloud.kubeform.com,resources=deliverychannels/status,verbs=get;update;patch

func (r *DeliveryChannelReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("deliverychannel", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DeliveryChannel")
...
func (r *DeliveryChannelReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &configv1alpha1.DeliveryChannel{}); err != nil {
			klog.Error(err, "unable to set up auditor", configv1alpha1.DeliveryChannel{}.APIVersion, configv1alpha1.DeliveryChannel{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&configv1alpha1.DeliveryChannel{}).
