...
// AggregateCompliancePackReconciler reconciles a AggregateCompliancePack object
type AggregateCompliancePackReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=config.alicloud.kubeform.com,resources=aggregatecompliancepacks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=config.alicloud.kubeform.com,resources=aggregatecompliancepacks/status,verbs=get;update;patch

func (r *AggregateCompliancePackReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("aggregatecompliancepack", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AggregateCompliancePack")
...
func (r *AggregateCompliancePackReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &configv1alpha1.AggregateCompliancePack{}); err != nil {
			klog.Error(err, "unable to set up auditor", configv1alpha1.AggregateCompliancePack{}.APIVersion, configv1alpha1.AggregateCompliancePack{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&configv1alpha1.AggregateCompliancePack{}).
