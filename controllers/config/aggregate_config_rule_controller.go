...
// AggregateConfigRuleReconciler reconciles a AggregateConfigRule object
type AggregateConfigRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=config.alicloud.kubeform.com,resources=aggregateconfigrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=config.alicloud.kubeform.com,resources=aggregateconfigrules/status,verbs=get;update;patch

func (r *AggregateConfigRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("aggregateconfigrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AggregateConfigRule")
...
func (r *AggregateConfigRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &configv1alpha1.AggregateConfigRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", configv1alpha1.AggregateConfigRule{}.APIVersion, configv1alpha1.AggregateConfigRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&configv1alpha1.AggregateConfigRule{}).
