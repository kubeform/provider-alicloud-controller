...
// CompliancePackReconciler reconciles a CompliancePack object
type CompliancePackReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=config.alicloud.kubeform.com,resources=compliancepacks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=config.alicloud.kubeform.com,resources=compliancepacks/status,verbs=get;update;patch

func (r *CompliancePackReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("compliancepack", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CompliancePack")
...
func (r *CompliancePackReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &configv1alpha1.CompliancePack{}); err != nil {
			klog.Error(err, "unable to set up auditor", configv1alpha1.CompliancePack{}.APIVersion, configv1alpha1.CompliancePack{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&configv1alpha1.CompliancePack{}).
