...
// ConfigurationRecorderReconciler reconciles a ConfigurationRecorder object
type ConfigurationRecorderReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=config.alicloud.kubeform.com,resources=configurationrecorders,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=config.alicloud.kubeform.com,resources=configurationrecorders/status,verbs=get;update;patch

func (r *ConfigurationRecorderReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("configurationrecorder", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ConfigurationRecorder")
...
func (r *ConfigurationRecorderReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &configv1alpha1.ConfigurationRecorder{}); err != nil {
			klog.Error(err, "unable to set up auditor", configv1alpha1.ConfigurationRecorder{}.APIVersion, configv1alpha1.ConfigurationRecorder{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&configv1alpha1.ConfigurationRecorder{}).
