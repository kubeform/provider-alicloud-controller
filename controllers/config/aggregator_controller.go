...
// AggregatorReconciler reconciles a Aggregator object
...
// +kubebuilder:rbac:groups=config.alicloud.kubeform.com,resources=aggregators,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=config.alicloud.kubeform.com,resources=aggregators/status,verbs=get;update;patch

func (r *AggregatorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("aggregator", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Aggregator")
...
func (r *AggregatorReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &configv1alpha1.Aggregator{}); err != nil {
			klog.Error(err, "unable to set up auditor", configv1alpha1.Aggregator{}.APIVersion, configv1alpha1.Aggregator{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&configv1alpha1.Aggregator{}).
