...
// K8sApplicationReconciler reconciles a K8sApplication object
type K8sApplicationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=edas.alicloud.kubeform.com,resources=k8sapplications,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=edas.alicloud.kubeform.com,resources=k8sapplications/status,verbs=get;update;patch

func (r *K8sApplicationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("k8sapplication", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch K8sApplication")
...
func (r *K8sApplicationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &edasv1alpha1.K8sApplication{}); err != nil {
			klog.Error(err, "unable to set up auditor", edasv1alpha1.K8sApplication{}.APIVersion, edasv1alpha1.K8sApplication{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&edasv1alpha1.K8sApplication{}).
