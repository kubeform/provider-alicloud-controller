...
// DeployGroupReconciler reconciles a DeployGroup object
type DeployGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=edas.alicloud.kubeform.com,resources=deploygroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=edas.alicloud.kubeform.com,resources=deploygroups/status,verbs=get;update;patch

func (r *DeployGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("deploygroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DeployGroup")
...
func (r *DeployGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &edasv1alpha1.DeployGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", edasv1alpha1.DeployGroup{}.APIVersion, edasv1alpha1.DeployGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&edasv1alpha1.DeployGroup{}).
