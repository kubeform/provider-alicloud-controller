...
// SlbAttachmentReconciler reconciles a SlbAttachment object
type SlbAttachmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=edas.alicloud.kubeform.com,resources=slbattachments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=edas.alicloud.kubeform.com,resources=slbattachments/status,verbs=get;update;patch

func (r *SlbAttachmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("slbattachment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SlbAttachment")
...
func (r *SlbAttachmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &edasv1alpha1.SlbAttachment{}); err != nil {
			klog.Error(err, "unable to set up auditor", edasv1alpha1.SlbAttachment{}.APIVersion, edasv1alpha1.SlbAttachment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&edasv1alpha1.SlbAttachment{}).
