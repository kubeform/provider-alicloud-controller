...
// +kubebuilder:rbac:groups=edas.alicloud.kubeform.com,resources=applicationdeployments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=edas.alicloud.kubeform.com,resources=applicationdeployments/status,verbs=get;update;patch

func (r *ApplicationDeploymentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("applicationdeployment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ApplicationDeployment")
...
func (r *ApplicationDeploymentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &edasv1alpha1.ApplicationDeployment{}); err != nil {
			klog.Error(err, "unable to set up auditor", edasv1alpha1.ApplicationDeployment{}.APIVersion, edasv1alpha1.ApplicationDeployment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&edasv1alpha1.ApplicationDeployment{}).
