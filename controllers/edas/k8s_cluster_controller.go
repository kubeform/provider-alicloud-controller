...
// +kubebuilder:rbac:groups=edas.alicloud.kubeform.com,resources=k8sclusters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=edas.alicloud.kubeform.com,resources=k8sclusters/status,verbs=get;update;patch

func (r *K8sClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("k8scluster", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch K8sCluster")
...
func (r *K8sClusterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &edasv1alpha1.K8sCluster{}); err != nil {
			klog.Error(err, "unable to set up auditor", edasv1alpha1.K8sCluster{}.APIVersion, edasv1alpha1.K8sCluster{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&edasv1alpha1.K8sCluster{}).
