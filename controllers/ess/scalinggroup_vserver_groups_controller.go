...
// ScalinggroupVserverGroupsReconciler reconciles a ScalinggroupVserverGroups object
type ScalinggroupVserverGroupsReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ess.alicloud.kubeform.com,resources=scalinggroupvservergroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ess.alicloud.kubeform.com,resources=scalinggroupvservergroups/status,verbs=get;update;patch

func (r *ScalinggroupVserverGroupsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("scalinggroupvservergroups", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ScalinggroupVserverGroups")
...
func (r *ScalinggroupVserverGroupsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &essv1alpha1.ScalinggroupVserverGroups{}); err != nil {
			klog.Error(err, "unable to set up auditor", essv1alpha1.ScalinggroupVserverGroups{}.APIVersion, essv1alpha1.ScalinggroupVserverGroups{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&essv1alpha1.ScalinggroupVserverGroups{}).
