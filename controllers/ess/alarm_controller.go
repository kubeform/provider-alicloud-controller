...
// +kubebuilder:rbac:groups=ess.alicloud.kubeform.com,resources=alarms,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ess.alicloud.kubeform.com,resources=alarms/status,verbs=get;update;patch

func (r *AlarmReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("alarm", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Alarm")
...
func (r *AlarmReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &essv1alpha1.Alarm{}); err != nil {
			klog.Error(err, "unable to set up auditor", essv1alpha1.Alarm{}.APIVersion, essv1alpha1.Alarm{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&essv1alpha1.Alarm{}).
