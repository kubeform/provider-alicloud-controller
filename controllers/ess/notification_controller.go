...
// +kubebuilder:rbac:groups=ess.alicloud.kubeform.com,resources=notifications,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ess.alicloud.kubeform.com,resources=notifications/status,verbs=get;update;patch
...
func (r *NotificationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Notification")
...
func (r *NotificationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &essv1alpha1.Notification{}); err != nil {
			klog.Error(err, "unable to set up auditor", essv1alpha1.Notification{}.APIVersion, essv1alpha1.Notification{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&essv1alpha1.Notification{}).
