...
// ScalingConfigurationReconciler reconciles a ScalingConfiguration object
type ScalingConfigurationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ess.alicloud.kubeform.com,resources=scalingconfigurations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ess.alicloud.kubeform.com,resources=scalingconfigurations/status,verbs=get;update;patch

func (r *ScalingConfigurationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("scalingconfiguration", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ScalingConfiguration")
...
func (r *ScalingConfigurationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &essv1alpha1.ScalingConfiguration{}); err != nil {
			klog.Error(err, "unable to set up auditor", essv1alpha1.ScalingConfiguration{}.APIVersion, essv1alpha1.ScalingConfiguration{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&essv1alpha1.ScalingConfiguration{}).
