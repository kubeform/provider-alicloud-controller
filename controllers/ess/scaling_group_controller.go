...
// +kubebuilder:rbac:groups=ess.alicloud.kubeform.com,resources=scalinggroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ess.alicloud.kubeform.com,resources=scalinggroups/status,verbs=get;update;patch

func (r *ScalingGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("scalinggroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ScalingGroup")
...
func (r *ScalingGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &essv1alpha1.ScalingGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", essv1alpha1.ScalingGroup{}.APIVersion, essv1alpha1.ScalingGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&essv1alpha1.ScalingGroup{}).
