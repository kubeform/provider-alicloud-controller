...
// +kubebuilder:rbac:groups=ga.alicloud.kubeform.com,resources=accelerators,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ga.alicloud.kubeform.com,resources=accelerators/status,verbs=get;update;patch

func (r *AcceleratorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("accelerator", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Accelerator")
...
func (r *AcceleratorReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &gav1alpha1.Accelerator{}); err != nil {
			klog.Error(err, "unable to set up auditor", gav1alpha1.Accelerator{}.APIVersion, gav1alpha1.Accelerator{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&gav1alpha1.Accelerator{}).
