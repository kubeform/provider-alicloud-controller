...
// AclAttachmentReconciler reconciles a AclAttachment object
type AclAttachmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ga.alicloud.kubeform.com,resources=aclattachments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ga.alicloud.kubeform.com,resources=aclattachments/status,verbs=get;update;patch

func (r *AclAttachmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("aclattachment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AclAttachment")
...
func (r *AclAttachmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &gav1alpha1.AclAttachment{}); err != nil {
			klog.Error(err, "unable to set up auditor", gav1alpha1.AclAttachment{}.APIVersion, gav1alpha1.AclAttachment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&gav1alpha1.AclAttachment{}).
