...
// +kubebuilder:rbac:groups=vpn.alicloud.kubeform.com,resources=customergateways,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vpn.alicloud.kubeform.com,resources=customergateways/status,verbs=get;update;patch

func (r *CustomerGatewayReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("customergateway", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CustomerGateway")
...
func (r *CustomerGatewayReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vpnv1alpha1.CustomerGateway{}); err != nil {
			klog.Error(err, "unable to set up auditor", vpnv1alpha1.CustomerGateway{}.APIVersion, vpnv1alpha1.CustomerGateway{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vpnv1alpha1.CustomerGateway{}).
