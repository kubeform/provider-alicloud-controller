...
// CaCertificateReconciler reconciles a CaCertificate object
type CaCertificateReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=slb.alicloud.kubeform.com,resources=cacertificates,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=slb.alicloud.kubeform.com,resources=cacertificates/status,verbs=get;update;patch

func (r *CaCertificateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("cacertificate", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CaCertificate")
...
func (r *CaCertificateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &slbv1alpha1.CaCertificate{}); err != nil {
			klog.Error(err, "unable to set up auditor", slbv1alpha1.CaCertificate{}.APIVersion, slbv1alpha1.CaCertificate{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&slbv1alpha1.CaCertificate{}).
