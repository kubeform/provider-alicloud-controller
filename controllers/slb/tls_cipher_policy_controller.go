...
// TlsCipherPolicyReconciler reconciles a TlsCipherPolicy object
type TlsCipherPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=slb.alicloud.kubeform.com,resources=tlscipherpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=slb.alicloud.kubeform.com,resources=tlscipherpolicies/status,verbs=get;update;patch

func (r *TlsCipherPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("tlscipherpolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TlsCipherPolicy")
...
func (r *TlsCipherPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &slbv1alpha1.TlsCipherPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", slbv1alpha1.TlsCipherPolicy{}.APIVersion, slbv1alpha1.TlsCipherPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&slbv1alpha1.TlsCipherPolicy{}).
