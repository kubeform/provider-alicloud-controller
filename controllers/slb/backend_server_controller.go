...
// BackendServerReconciler reconciles a BackendServer object
type BackendServerReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=slb.alicloud.kubeform.com,resources=backendservers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=slb.alicloud.kubeform.com,resources=backendservers/status,verbs=get;update;patch

func (r *BackendServerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("backendserver", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BackendServer")
...
func (r *BackendServerReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &slbv1alpha1.BackendServer{}); err != nil {
			klog.Error(err, "unable to set up auditor", slbv1alpha1.BackendServer{}.APIVersion, slbv1alpha1.BackendServer{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&slbv1alpha1.BackendServer{}).
