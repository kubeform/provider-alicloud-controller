...
// SlbReconciler reconciles a Slb object
type SlbReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=slb.alicloud.kubeform.com,resources=slbs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=slb.alicloud.kubeform.com,resources=slbs/status,verbs=get;update;patch

func (r *SlbReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("slb", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Slb")
...
func (r *SlbReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &slbv1alpha1.Slb{}); err != nil {
			klog.Error(err, "unable to set up auditor", slbv1alpha1.Slb{}.APIVersion, slbv1alpha1.Slb{}.Kind)
			return err
		}
	}
