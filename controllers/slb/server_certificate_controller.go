...
// ServerCertificateReconciler reconciles a ServerCertificate object
type ServerCertificateReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=slb.alicloud.kubeform.com,resources=servercertificates,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=slb.alicloud.kubeform.com,resources=servercertificates/status,verbs=get;update;patch

func (r *ServerCertificateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("servercertificate", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServerCertificate")
...
func (r *ServerCertificateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &slbv1alpha1.ServerCertificate{}); err != nil {
			klog.Error(err, "unable to set up auditor", slbv1alpha1.ServerCertificate{}.APIVersion, slbv1alpha1.ServerCertificate{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&slbv1alpha1.ServerCertificate{}).
