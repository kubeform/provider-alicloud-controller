...
// BackupPlanReconciler reconciles a BackupPlan object
...
// +kubebuilder:rbac:groups=cassandra.alicloud.kubeform.com,resources=backupplans,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cassandra.alicloud.kubeform.com,resources=backupplans/status,verbs=get;update;patch

func (r *BackupPlanReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("backupplan", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BackupPlan")
...
func (r *BackupPlanReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cassandrav1alpha1.BackupPlan{}); err != nil {
			klog.Error(err, "unable to set up auditor", cassandrav1alpha1.BackupPlan{}.APIVersion, cassandrav1alpha1.BackupPlan{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cassandrav1alpha1.BackupPlan{}).
