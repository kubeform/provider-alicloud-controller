...
// +kubebuilder:rbac:groups=adb.alicloud.kubeform.com,resources=dbclusters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=adb.alicloud.kubeform.com,resources=dbclusters/status,verbs=get;update;patch

func (r *DbClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dbcluster", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DbCluster")
...
func (r *DbClusterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &adbv1alpha1.DbCluster{}); err != nil {
			klog.Error(err, "unable to set up auditor", adbv1alpha1.DbCluster{}.APIVersion, adbv1alpha1.DbCluster{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&adbv1alpha1.DbCluster{}).
