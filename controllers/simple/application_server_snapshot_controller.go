...
// ApplicationServerSnapshotReconciler reconciles a ApplicationServerSnapshot object
type ApplicationServerSnapshotReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=simple.alicloud.kubeform.com,resources=applicationserversnapshots,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=simple.alicloud.kubeform.com,resources=applicationserversnapshots/status,verbs=get;update;patch

func (r *ApplicationServerSnapshotReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("applicationserversnapshot", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ApplicationServerSnapshot")
...
func (r *ApplicationServerSnapshotReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &simplev1alpha1.ApplicationServerSnapshot{}); err != nil {
			klog.Error(err, "unable to set up auditor", simplev1alpha1.ApplicationServerSnapshot{}.APIVersion, simplev1alpha1.ApplicationServerSnapshot{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&simplev1alpha1.ApplicationServerSnapshot{}).
