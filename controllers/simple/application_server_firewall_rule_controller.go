...
// ApplicationServerFirewallRuleReconciler reconciles a ApplicationServerFirewallRule object
type ApplicationServerFirewallRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=simple.alicloud.kubeform.com,resources=applicationserverfirewallrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=simple.alicloud.kubeform.com,resources=applicationserverfirewallrules/status,verbs=get;update;patch

func (r *ApplicationServerFirewallRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("applicationserverfirewallrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ApplicationServerFirewallRule")
...
func (r *ApplicationServerFirewallRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &simplev1alpha1.ApplicationServerFirewallRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", simplev1alpha1.ApplicationServerFirewallRule{}.APIVersion, simplev1alpha1.ApplicationServerFirewallRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&simplev1alpha1.ApplicationServerFirewallRule{}).
