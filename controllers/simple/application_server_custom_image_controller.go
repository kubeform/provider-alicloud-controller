...
// ApplicationServerCustomImageReconciler reconciles a ApplicationServerCustomImage object
type ApplicationServerCustomImageReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=simple.alicloud.kubeform.com,resources=applicationservercustomimages,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=simple.alicloud.kubeform.com,resources=applicationservercustomimages/status,verbs=get;update;patch

func (r *ApplicationServerCustomImageReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("applicationservercustomimage", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ApplicationServerCustomImage")
...
func (r *ApplicationServerCustomImageReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &simplev1alpha1.ApplicationServerCustomImage{}); err != nil {
			klog.Error(err, "unable to set up auditor", simplev1alpha1.ApplicationServerCustomImage{}.APIVersion, simplev1alpha1.ApplicationServerCustomImage{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&simplev1alpha1.ApplicationServerCustomImage{}).
