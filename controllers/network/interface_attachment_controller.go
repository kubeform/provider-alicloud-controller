...
// InterfaceAttachmentReconciler reconciles a InterfaceAttachment object
type InterfaceAttachmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=network.alicloud.kubeform.com,resources=interfaceattachments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=network.alicloud.kubeform.com,resources=interfaceattachments/status,verbs=get;update;patch

func (r *InterfaceAttachmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("interfaceattachment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch InterfaceAttachment")
...
func (r *InterfaceAttachmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &networkv1alpha1.InterfaceAttachment{}); err != nil {
			klog.Error(err, "unable to set up auditor", networkv1alpha1.InterfaceAttachment{}.APIVersion, networkv1alpha1.InterfaceAttachment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&networkv1alpha1.InterfaceAttachment{}).
