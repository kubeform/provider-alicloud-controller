...
// +kubebuilder:rbac:groups=kvstore.alicloud.kubeform.com,resources=auditlogconfigs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kvstore.alicloud.kubeform.com,resources=auditlogconfigs/status,verbs=get;update;patch

func (r *AuditLogConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("auditlogconfig", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AuditLogConfig")
...
func (r *AuditLogConfigReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kvstorev1alpha1.AuditLogConfig{}); err != nil {
			klog.Error(err, "unable to set up auditor", kvstorev1alpha1.AuditLogConfig{}.APIVersion, kvstorev1alpha1.AuditLogConfig{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kvstorev1alpha1.AuditLogConfig{}).
