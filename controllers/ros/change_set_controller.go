...
// ChangeSetReconciler reconciles a ChangeSet object
...
// +kubebuilder:rbac:groups=ros.alicloud.kubeform.com,resources=changesets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ros.alicloud.kubeform.com,resources=changesets/status,verbs=get;update;patch

func (r *ChangeSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("changeset", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ChangeSet")
...
func (r *ChangeSetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &rosv1alpha1.ChangeSet{}); err != nil {
			klog.Error(err, "unable to set up auditor", rosv1alpha1.ChangeSet{}.APIVersion, rosv1alpha1.ChangeSet{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&rosv1alpha1.ChangeSet{}).
