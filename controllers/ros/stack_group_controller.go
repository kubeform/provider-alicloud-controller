...
// StackGroupReconciler reconciles a StackGroup object
type StackGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ros.alicloud.kubeform.com,resources=stackgroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ros.alicloud.kubeform.com,resources=stackgroups/status,verbs=get;update;patch

func (r *StackGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("stackgroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch StackGroup")
...
func (r *StackGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &rosv1alpha1.StackGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", rosv1alpha1.StackGroup{}.APIVersion, rosv1alpha1.StackGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&rosv1alpha1.StackGroup{}).
