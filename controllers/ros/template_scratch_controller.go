...
// TemplateScratchReconciler reconciles a TemplateScratch object
type TemplateScratchReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ros.alicloud.kubeform.com,resources=templatescratches,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ros.alicloud.kubeform.com,resources=templatescratches/status,verbs=get;update;patch

func (r *TemplateScratchReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("templatescratch", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TemplateScratch")
...
func (r *TemplateScratchReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &rosv1alpha1.TemplateScratch{}); err != nil {
			klog.Error(err, "unable to set up auditor", rosv1alpha1.TemplateScratch{}.APIVersion, rosv1alpha1.TemplateScratch{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&rosv1alpha1.TemplateScratch{}).
