...
// +kubebuilder:rbac:groups=ros.alicloud.kubeform.com,resources=stackinstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ros.alicloud.kubeform.com,resources=stackinstances/status,verbs=get;update;patch

func (r *StackInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("stackinstance", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch StackInstance")
...
func (r *StackInstanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &rosv1alpha1.StackInstance{}); err != nil {
			klog.Error(err, "unable to set up auditor", rosv1alpha1.StackInstance{}.APIVersion, rosv1alpha1.StackInstance{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&rosv1alpha1.StackInstance{}).
...
func (r *StackInstanceReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for StackInstance")
			return nil
		}
		...
		resourceList := &rosv1alpha1.StackInstanceList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list StackInstance")
			return nil
		}
