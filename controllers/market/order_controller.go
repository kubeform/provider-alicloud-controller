...
// +kubebuilder:rbac:groups=market.alicloud.kubeform.com,resources=orders,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=market.alicloud.kubeform.com,resources=orders/status,verbs=get;update;patch

func (r *OrderReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("order", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Order")
...
func (r *OrderReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &marketv1alpha1.Order{}); err != nil {
			klog.Error(err, "unable to set up auditor", marketv1alpha1.Order{}.APIVersion, marketv1alpha1.Order{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&marketv1alpha1.Order{}).
