...
// DedicatedHostGroupReconciler reconciles a DedicatedHostGroup object
type DedicatedHostGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cddc.alicloud.kubeform.com,resources=dedicatedhostgroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cddc.alicloud.kubeform.com,resources=dedicatedhostgroups/status,verbs=get;update;patch

func (r *DedicatedHostGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dedicatedhostgroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DedicatedHostGroup")
...
func (r *DedicatedHostGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cddcv1alpha1.DedicatedHostGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", cddcv1alpha1.DedicatedHostGroup{}.APIVersion, cddcv1alpha1.DedicatedHostGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cddcv1alpha1.DedicatedHostGroup{}).
