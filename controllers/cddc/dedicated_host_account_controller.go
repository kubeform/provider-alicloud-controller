...
// DedicatedHostAccountReconciler reconciles a DedicatedHostAccount object
type DedicatedHostAccountReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cddc.alicloud.kubeform.com,resources=dedicatedhostaccounts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cddc.alicloud.kubeform.com,resources=dedicatedhostaccounts/status,verbs=get;update;patch

func (r *DedicatedHostAccountReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dedicatedhostaccount", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DedicatedHostAccount")
...
func (r *DedicatedHostAccountReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cddcv1alpha1.DedicatedHostAccount{}); err != nil {
			klog.Error(err, "unable to set up auditor", cddcv1alpha1.DedicatedHostAccount{}.APIVersion, cddcv1alpha1.DedicatedHostAccount{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cddcv1alpha1.DedicatedHostAccount{}).
...
func (r *DedicatedHostAccountReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for DedicatedHostAccount")
			return nil
		}
		...
		resourceList := &cddcv1alpha1.DedicatedHostAccountList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list DedicatedHostAccount")
			return nil
		}
