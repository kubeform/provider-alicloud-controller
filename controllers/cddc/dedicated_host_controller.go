...
// DedicatedHostReconciler reconciles a DedicatedHost object
type DedicatedHostReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cddc.alicloud.kubeform.com,resources=dedicatedhosts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cddc.alicloud.kubeform.com,resources=dedicatedhosts/status,verbs=get;update;patch

func (r *DedicatedHostReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dedicatedhost", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DedicatedHost")
...
func (r *DedicatedHostReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cddcv1alpha1.DedicatedHost{}); err != nil {
			klog.Error(err, "unable to set up auditor", cddcv1alpha1.DedicatedHost{}.APIVersion, cddcv1alpha1.DedicatedHost{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cddcv1alpha1.DedicatedHost{}).
...
func (r *DedicatedHostReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for DedicatedHost")
			return nil
		}
		...
		resourceList := &cddcv1alpha1.DedicatedHostList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list DedicatedHost")
			return nil
		}
