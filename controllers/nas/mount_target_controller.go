...
// MountTargetReconciler reconciles a MountTarget object
type MountTargetReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=nas.alicloud.kubeform.com,resources=mounttargets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=nas.alicloud.kubeform.com,resources=mounttargets/status,verbs=get;update;patch

func (r *MountTargetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("mounttarget", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MountTarget")
...
func (r *MountTargetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &nasv1alpha1.MountTarget{}); err != nil {
			klog.Error(err, "unable to set up auditor", nasv1alpha1.MountTarget{}.APIVersion, nasv1alpha1.MountTarget{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&nasv1alpha1.MountTarget{}).
