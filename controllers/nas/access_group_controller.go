...
// AccessGroupReconciler reconciles a AccessGroup object
...
// +kubebuilder:rbac:groups=nas.alicloud.kubeform.com,resources=accessgroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=nas.alicloud.kubeform.com,resources=accessgroups/status,verbs=get;update;patch

func (r *AccessGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("accessgroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AccessGroup")
...
func (r *AccessGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &nasv1alpha1.AccessGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", nasv1alpha1.AccessGroup{}.APIVersion, nasv1alpha1.AccessGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&nasv1alpha1.AccessGroup{}).
