...
// +kubebuilder:rbac:groups=nas.alicloud.kubeform.com,resources=filesets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=nas.alicloud.kubeform.com,resources=filesets/status,verbs=get;update;patch

func (r *FilesetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("fileset", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Fileset")
...
func (r *FilesetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &nasv1alpha1.Fileset{}); err != nil {
			klog.Error(err, "unable to set up auditor", nasv1alpha1.Fileset{}.APIVersion, nasv1alpha1.Fileset{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&nasv1alpha1.Fileset{}).
