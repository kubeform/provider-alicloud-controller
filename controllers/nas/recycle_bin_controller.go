...
// RecycleBinReconciler reconciles a RecycleBin object
...
// +kubebuilder:rbac:groups=nas.alicloud.kubeform.com,resources=recyclebins,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=nas.alicloud.kubeform.com,resources=recyclebins/status,verbs=get;update;patch

func (r *RecycleBinReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("recyclebin", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RecycleBin")
...
func (r *RecycleBinReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &nasv1alpha1.RecycleBin{}); err != nil {
			klog.Error(err, "unable to set up auditor", nasv1alpha1.RecycleBin{}.APIVersion, nasv1alpha1.RecycleBin{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&nasv1alpha1.RecycleBin{}).
