...
// +kubebuilder:rbac:groups=nas.alicloud.kubeform.com,resources=lifecyclepolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=nas.alicloud.kubeform.com,resources=lifecyclepolicies/status,verbs=get;update;patch

func (r *LifecyclePolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("lifecyclepolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LifecyclePolicy")
...
func (r *LifecyclePolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &nasv1alpha1.LifecyclePolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", nasv1alpha1.LifecyclePolicy{}.APIVersion, nasv1alpha1.LifecyclePolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&nasv1alpha1.LifecyclePolicy{}).
