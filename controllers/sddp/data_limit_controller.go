...
// DataLimitReconciler reconciles a DataLimit object
type DataLimitReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=sddp.alicloud.kubeform.com,resources=datalimits,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sddp.alicloud.kubeform.com,resources=datalimits/status,verbs=get;update;patch

func (r *DataLimitReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("datalimit", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DataLimit")
...
func (r *DataLimitReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sddpv1alpha1.DataLimit{}); err != nil {
			klog.Error(err, "unable to set up auditor", sddpv1alpha1.DataLimit{}.APIVersion, sddpv1alpha1.DataLimit{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sddpv1alpha1.DataLimit{}).
