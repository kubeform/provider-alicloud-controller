...
// GtmInstanceReconciler reconciles a GtmInstance object
type GtmInstanceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=alidns.alicloud.kubeform.com,resources=gtminstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=alidns.alicloud.kubeform.com,resources=gtminstances/status,verbs=get;update;patch

func (r *GtmInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("gtminstance", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GtmInstance")
...
func (r *GtmInstanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &alidnsv1alpha1.GtmInstance{}); err != nil {
			klog.Error(err, "unable to set up auditor", alidnsv1alpha1.GtmInstance{}.APIVersion, alidnsv1alpha1.GtmInstance{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&alidnsv1alpha1.GtmInstance{}).
