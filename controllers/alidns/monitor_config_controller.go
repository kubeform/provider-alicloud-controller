...
// +kubebuilder:rbac:groups=alidns.alicloud.kubeform.com,resources=monitorconfigs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=alidns.alicloud.kubeform.com,resources=monitorconfigs/status,verbs=get;update;patch

func (r *MonitorConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("monitorconfig", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MonitorConfig")
...
func (r *MonitorConfigReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &alidnsv1alpha1.MonitorConfig{}); err != nil {
			klog.Error(err, "unable to set up auditor", alidnsv1alpha1.MonitorConfig{}.APIVersion, alidnsv1alpha1.MonitorConfig{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&alidnsv1alpha1.MonitorConfig{}).
