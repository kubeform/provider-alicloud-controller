...
// AccessStrategyReconciler reconciles a AccessStrategy object
type AccessStrategyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=alidns.alicloud.kubeform.com,resources=accessstrategies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=alidns.alicloud.kubeform.com,resources=accessstrategies/status,verbs=get;update;patch

func (r *AccessStrategyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("accessstrategy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AccessStrategy")
...
func (r *AccessStrategyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &alidnsv1alpha1.AccessStrategy{}); err != nil {
			klog.Error(err, "unable to set up auditor", alidnsv1alpha1.AccessStrategy{}.APIVersion, alidnsv1alpha1.AccessStrategy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&alidnsv1alpha1.AccessStrategy{}).
