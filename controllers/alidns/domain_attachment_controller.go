...
// DomainAttachmentReconciler reconciles a DomainAttachment object
type DomainAttachmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=alidns.alicloud.kubeform.com,resources=domainattachments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=alidns.alicloud.kubeform.com,resources=domainattachments/status,verbs=get;update;patch

func (r *DomainAttachmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("domainattachment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DomainAttachment")
...
func (r *DomainAttachmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &alidnsv1alpha1.DomainAttachment{}); err != nil {
			klog.Error(err, "unable to set up auditor", alidnsv1alpha1.DomainAttachment{}.APIVersion, alidnsv1alpha1.DomainAttachment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&alidnsv1alpha1.DomainAttachment{}).
