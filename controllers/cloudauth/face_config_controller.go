...
// FaceConfigReconciler reconciles a FaceConfig object
type FaceConfigReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloudauth.alicloud.kubeform.com,resources=faceconfigs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloudauth.alicloud.kubeform.com,resources=faceconfigs/status,verbs=get;update;patch

func (r *FaceConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("faceconfig", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FaceConfig")
...
func (r *FaceConfigReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudauthv1alpha1.FaceConfig{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudauthv1alpha1.FaceConfig{}.APIVersion, cloudauthv1alpha1.FaceConfig{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudauthv1alpha1.FaceConfig{}).
