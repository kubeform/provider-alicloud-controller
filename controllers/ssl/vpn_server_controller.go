...
// VpnServerReconciler reconciles a VpnServer object
...
// +kubebuilder:rbac:groups=ssl.alicloud.kubeform.com,resources=vpnservers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ssl.alicloud.kubeform.com,resources=vpnservers/status,verbs=get;update;patch

func (r *VpnServerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vpnserver", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VpnServer")
...
func (r *VpnServerReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sslv1alpha1.VpnServer{}); err != nil {
			klog.Error(err, "unable to set up auditor", sslv1alpha1.VpnServer{}.APIVersion, sslv1alpha1.VpnServer{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sslv1alpha1.VpnServer{}).
