...
// VpnClientCertReconciler reconciles a VpnClientCert object
type VpnClientCertReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ssl.alicloud.kubeform.com,resources=vpnclientcerts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ssl.alicloud.kubeform.com,resources=vpnclientcerts/status,verbs=get;update;patch

func (r *VpnClientCertReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vpnclientcert", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VpnClientCert")
...
func (r *VpnClientCertReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sslv1alpha1.VpnClientCert{}); err != nil {
			klog.Error(err, "unable to set up auditor", sslv1alpha1.VpnClientCert{}.APIVersion, sslv1alpha1.VpnClientCert{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sslv1alpha1.VpnClientCert{}).
...
func (r *VpnClientCertReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for VpnClientCert")
			return nil
		}
		...
		resourceList := &sslv1alpha1.VpnClientCertList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list VpnClientCert")
			return nil
		}
