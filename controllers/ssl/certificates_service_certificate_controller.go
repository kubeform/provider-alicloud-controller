...
// CertificatesServiceCertificateReconciler reconciles a CertificatesServiceCertificate object
type CertificatesServiceCertificateReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ssl.alicloud.kubeform.com,resources=certificatesservicecertificates,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ssl.alicloud.kubeform.com,resources=certificatesservicecertificates/status,verbs=get;update;patch

func (r *CertificatesServiceCertificateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("certificatesservicecertificate", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CertificatesServiceCertificate")
...
func (r *CertificatesServiceCertificateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sslv1alpha1.CertificatesServiceCertificate{}); err != nil {
			klog.Error(err, "unable to set up auditor", sslv1alpha1.CertificatesServiceCertificate{}.APIVersion, sslv1alpha1.CertificatesServiceCertificate{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sslv1alpha1.CertificatesServiceCertificate{}).
