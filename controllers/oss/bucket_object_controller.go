...
// BucketObjectReconciler reconciles a BucketObject object
type BucketObjectReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=oss.alicloud.kubeform.com,resources=bucketobjects,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=oss.alicloud.kubeform.com,resources=bucketobjects/status,verbs=get;update;patch

func (r *BucketObjectReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("bucketobject", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BucketObject")
...
func (r *BucketObjectReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ossv1alpha1.BucketObject{}); err != nil {
			klog.Error(err, "unable to set up auditor", ossv1alpha1.BucketObject{}.APIVersion, ossv1alpha1.BucketObject{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ossv1alpha1.BucketObject{}).
