...
// TrailReconciler reconciles a Trail object
...
// +kubebuilder:rbac:groups=actiontrail.alicloud.kubeform.com,resources=trails,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=actiontrail.alicloud.kubeform.com,resources=trails/status,verbs=get;update;patch

func (r *TrailReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("trail", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Trail")
...
func (r *TrailReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &actiontrailv1alpha1.Trail{}); err != nil {
			klog.Error(err, "unable to set up auditor", actiontrailv1alpha1.Trail{}.APIVersion, actiontrailv1alpha1.Trail{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&actiontrailv1alpha1.Trail{}).
