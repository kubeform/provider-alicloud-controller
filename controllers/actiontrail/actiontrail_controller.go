...
// ActiontrailReconciler reconciles a Actiontrail object
type ActiontrailReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=actiontrail.alicloud.kubeform.com,resources=actiontrails,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=actiontrail.alicloud.kubeform.com,resources=actiontrails/status,verbs=get;update;patch

func (r *ActiontrailReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("actiontrail", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Actiontrail")
...
func (r *ActiontrailReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &actiontrailv1alpha1.Actiontrail{}); err != nil {
			klog.Error(err, "unable to set up auditor", actiontrailv1alpha1.Actiontrail{}.APIVersion, actiontrailv1alpha1.Actiontrail{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&actiontrailv1alpha1.Actiontrail{}).
