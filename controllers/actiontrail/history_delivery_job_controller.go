...
// HistoryDeliveryJobReconciler reconciles a HistoryDeliveryJob object
type HistoryDeliveryJobReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=actiontrail.alicloud.kubeform.com,resources=historydeliveryjobs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=actiontrail.alicloud.kubeform.com,resources=historydeliveryjobs/status,verbs=get;update;patch

func (r *HistoryDeliveryJobReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("historydeliveryjob", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch HistoryDeliveryJob")
...
func (r *HistoryDeliveryJobReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &actiontrailv1alpha1.HistoryDeliveryJob{}); err != nil {
			klog.Error(err, "unable to set up auditor", actiontrailv1alpha1.HistoryDeliveryJob{}.APIVersion, actiontrailv1alpha1.HistoryDeliveryJob{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&actiontrailv1alpha1.HistoryDeliveryJob{}).
