...
// FunctionAsyncInvokeConfigReconciler reconciles a FunctionAsyncInvokeConfig object
type FunctionAsyncInvokeConfigReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=fc.alicloud.kubeform.com,resources=functionasyncinvokeconfigs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=fc.alicloud.kubeform.com,resources=functionasyncinvokeconfigs/status,verbs=get;update;patch

func (r *FunctionAsyncInvokeConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("functionasyncinvokeconfig", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FunctionAsyncInvokeConfig")
...
func (r *FunctionAsyncInvokeConfigReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &fcv1alpha1.FunctionAsyncInvokeConfig{}); err != nil {
			klog.Error(err, "unable to set up auditor", fcv1alpha1.FunctionAsyncInvokeConfig{}.APIVersion, fcv1alpha1.FunctionAsyncInvokeConfig{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&fcv1alpha1.FunctionAsyncInvokeConfig{}).
