...
// +kubebuilder:rbac:groups=fc.alicloud.kubeform.com,resources=aliases,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=fc.alicloud.kubeform.com,resources=aliases/status,verbs=get;update;patch

func (r *AliasReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("alias", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Alias")
...
func (r *AliasReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &fcv1alpha1.Alias{}); err != nil {
			klog.Error(err, "unable to set up auditor", fcv1alpha1.Alias{}.APIVersion, fcv1alpha1.Alias{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&fcv1alpha1.Alias{}).
