...
// +kubebuilder:rbac:groups=privatelink.alicloud.kubeform.com,resources=vpcendpoints,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=privatelink.alicloud.kubeform.com,resources=vpcendpoints/status,verbs=get;update;patch

func (r *VpcEndpointReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vpcendpoint", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VpcEndpoint")
...
func (r *VpcEndpointReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &privatelinkv1alpha1.VpcEndpoint{}); err != nil {
			klog.Error(err, "unable to set up auditor", privatelinkv1alpha1.VpcEndpoint{}.APIVersion, privatelinkv1alpha1.VpcEndpoint{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&privatelinkv1alpha1.VpcEndpoint{}).
