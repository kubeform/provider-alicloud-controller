...
// VpcEndpointServiceUserReconciler reconciles a VpcEndpointServiceUser object
type VpcEndpointServiceUserReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=privatelink.alicloud.kubeform.com,resources=vpcendpointserviceusers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=privatelink.alicloud.kubeform.com,resources=vpcendpointserviceusers/status,verbs=get;update;patch

func (r *VpcEndpointServiceUserReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vpcendpointserviceuser", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VpcEndpointServiceUser")
...
func (r *VpcEndpointServiceUserReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &privatelinkv1alpha1.VpcEndpointServiceUser{}); err != nil {
			klog.Error(err, "unable to set up auditor", privatelinkv1alpha1.VpcEndpointServiceUser{}.APIVersion, privatelinkv1alpha1.VpcEndpointServiceUser{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&privatelinkv1alpha1.VpcEndpointServiceUser{}).
