...
// VpcEndpointZoneReconciler reconciles a VpcEndpointZone object
type VpcEndpointZoneReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=privatelink.alicloud.kubeform.com,resources=vpcendpointzones,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=privatelink.alicloud.kubeform.com,resources=vpcendpointzones/status,verbs=get;update;patch

func (r *VpcEndpointZoneReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vpcendpointzone", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VpcEndpointZone")
...
func (r *VpcEndpointZoneReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &privatelinkv1alpha1.VpcEndpointZone{}); err != nil {
			klog.Error(err, "unable to set up auditor", privatelinkv1alpha1.VpcEndpointZone{}.APIVersion, privatelinkv1alpha1.VpcEndpointZone{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&privatelinkv1alpha1.VpcEndpointZone{}).
