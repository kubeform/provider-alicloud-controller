...
// VpcEndpointServiceResourceReconciler reconciles a VpcEndpointServiceResource object
type VpcEndpointServiceResourceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=privatelink.alicloud.kubeform.com,resources=vpcendpointserviceresources,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=privatelink.alicloud.kubeform.com,resources=vpcendpointserviceresources/status,verbs=get;update;patch

func (r *VpcEndpointServiceResourceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vpcendpointserviceresource", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VpcEndpointServiceResource")
...
func (r *VpcEndpointServiceResourceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &privatelinkv1alpha1.VpcEndpointServiceResource{}); err != nil {
			klog.Error(err, "unable to set up auditor", privatelinkv1alpha1.VpcEndpointServiceResource{}.APIVersion, privatelinkv1alpha1.VpcEndpointServiceResource{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&privatelinkv1alpha1.VpcEndpointServiceResource{}).
