...
// +kubebuilder:rbac:groups=privatelink.alicloud.kubeform.com,resources=vpcendpointservices,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=privatelink.alicloud.kubeform.com,resources=vpcendpointservices/status,verbs=get;update;patch

func (r *VpcEndpointServiceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vpcendpointservice", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VpcEndpointService")
...
func (r *VpcEndpointServiceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &privatelinkv1alpha1.VpcEndpointService{}); err != nil {
			klog.Error(err, "unable to set up auditor", privatelinkv1alpha1.VpcEndpointService{}.APIVersion, privatelinkv1alpha1.VpcEndpointService{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&privatelinkv1alpha1.VpcEndpointService{}).
