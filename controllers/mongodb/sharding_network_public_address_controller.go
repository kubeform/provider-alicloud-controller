...
// ShardingNetworkPublicAddressReconciler reconciles a ShardingNetworkPublicAddress object
type ShardingNetworkPublicAddressReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=mongodb.alicloud.kubeform.com,resources=shardingnetworkpublicaddresses,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=mongodb.alicloud.kubeform.com,resources=shardingnetworkpublicaddresses/status,verbs=get;update;patch

func (r *ShardingNetworkPublicAddressReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("shardingnetworkpublicaddress", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ShardingNetworkPublicAddress")
...
func (r *ShardingNetworkPublicAddressReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &mongodbv1alpha1.ShardingNetworkPublicAddress{}); err != nil {
			klog.Error(err, "unable to set up auditor", mongodbv1alpha1.ShardingNetworkPublicAddress{}.APIVersion, mongodbv1alpha1.ShardingNetworkPublicAddress{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&mongodbv1alpha1.ShardingNetworkPublicAddress{}).
