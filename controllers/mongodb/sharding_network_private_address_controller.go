...
// ShardingNetworkPrivateAddressReconciler reconciles a ShardingNetworkPrivateAddress object
type ShardingNetworkPrivateAddressReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=mongodb.alicloud.kubeform.com,resources=shardingnetworkprivateaddresses,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=mongodb.alicloud.kubeform.com,resources=shardingnetworkprivateaddresses/status,verbs=get;update;patch

func (r *ShardingNetworkPrivateAddressReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("shardingnetworkprivateaddress", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ShardingNetworkPrivateAddress")
...
func (r *ShardingNetworkPrivateAddressReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &mongodbv1alpha1.ShardingNetworkPrivateAddress{}); err != nil {
			klog.Error(err, "unable to set up auditor", mongodbv1alpha1.ShardingNetworkPrivateAddress{}.APIVersion, mongodbv1alpha1.ShardingNetworkPrivateAddress{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&mongodbv1alpha1.ShardingNetworkPrivateAddress{}).
...
func (r *ShardingNetworkPrivateAddressReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for ShardingNetworkPrivateAddress")
			return nil
		}
		...
		resourceList := &mongodbv1alpha1.ShardingNetworkPrivateAddressList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list ShardingNetworkPrivateAddress")
			return nil
		}
