...
// ShardingInstanceReconciler reconciles a ShardingInstance object
type ShardingInstanceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=mongodb.alicloud.kubeform.com,resources=shardinginstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=mongodb.alicloud.kubeform.com,resources=shardinginstances/status,verbs=get;update;patch

func (r *ShardingInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("shardinginstance", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ShardingInstance")
...
func (r *ShardingInstanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &mongodbv1alpha1.ShardingInstance{}); err != nil {
			klog.Error(err, "unable to set up auditor", mongodbv1alpha1.ShardingInstance{}.APIVersion, mongodbv1alpha1.ShardingInstance{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&mongodbv1alpha1.ShardingInstance{}).
...
func (r *ShardingInstanceReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for ShardingInstance")
			return nil
		}
		...
		resourceList := &mongodbv1alpha1.ShardingInstanceList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list ShardingInstance")
			return nil
		}
