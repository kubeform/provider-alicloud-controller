...
// DeviceGroupReconciler reconciles a DeviceGroup object
type DeviceGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=iot.alicloud.kubeform.com,resources=devicegroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iot.alicloud.kubeform.com,resources=devicegroups/status,verbs=get;update;patch

func (r *DeviceGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("devicegroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DeviceGroup")
...
func (r *DeviceGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &iotv1alpha1.DeviceGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", iotv1alpha1.DeviceGroup{}.APIVersion, iotv1alpha1.DeviceGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&iotv1alpha1.DeviceGroup{}).
