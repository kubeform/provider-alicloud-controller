...
// MetricRuleTemplateReconciler reconciles a MetricRuleTemplate object
type MetricRuleTemplateReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cms.alicloud.kubeform.com,resources=metricruletemplates,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cms.alicloud.kubeform.com,resources=metricruletemplates/status,verbs=get;update;patch

func (r *MetricRuleTemplateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("metricruletemplate", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MetricRuleTemplate")
...
func (r *MetricRuleTemplateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cmsv1alpha1.MetricRuleTemplate{}); err != nil {
			klog.Error(err, "unable to set up auditor", cmsv1alpha1.MetricRuleTemplate{}.APIVersion, cmsv1alpha1.MetricRuleTemplate{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cmsv1alpha1.MetricRuleTemplate{}).
