...
// MonitorGroupReconciler reconciles a MonitorGroup object
...
// +kubebuilder:rbac:groups=cms.alicloud.kubeform.com,resources=monitorgroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cms.alicloud.kubeform.com,resources=monitorgroups/status,verbs=get;update;patch

func (r *MonitorGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("monitorgroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MonitorGroup")
...
func (r *MonitorGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cmsv1alpha1.MonitorGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", cmsv1alpha1.MonitorGroup{}.APIVersion, cmsv1alpha1.MonitorGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cmsv1alpha1.MonitorGroup{}).
