...
// DynamicTagGroupReconciler reconciles a DynamicTagGroup object
type DynamicTagGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cms.alicloud.kubeform.com,resources=dynamictaggroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cms.alicloud.kubeform.com,resources=dynamictaggroups/status,verbs=get;update;patch

func (r *DynamicTagGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dynamictaggroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DynamicTagGroup")
...
func (r *DynamicTagGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cmsv1alpha1.DynamicTagGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", cmsv1alpha1.DynamicTagGroup{}.APIVersion, cmsv1alpha1.DynamicTagGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cmsv1alpha1.DynamicTagGroup{}).
