...
// MonitorGroupInstancesReconciler reconciles a MonitorGroupInstances object
type MonitorGroupInstancesReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cms.alicloud.kubeform.com,resources=monitorgroupinstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cms.alicloud.kubeform.com,resources=monitorgroupinstances/status,verbs=get;update;patch

func (r *MonitorGroupInstancesReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("monitorgroupinstances", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MonitorGroupInstances")
...
func (r *MonitorGroupInstancesReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cmsv1alpha1.MonitorGroupInstances{}); err != nil {
			klog.Error(err, "unable to set up auditor", cmsv1alpha1.MonitorGroupInstances{}.APIVersion, cmsv1alpha1.MonitorGroupInstances{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cmsv1alpha1.MonitorGroupInstances{}).
