...
// AlarmContactGroupReconciler reconciles a AlarmContactGroup object
type AlarmContactGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cms.alicloud.kubeform.com,resources=alarmcontactgroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cms.alicloud.kubeform.com,resources=alarmcontactgroups/status,verbs=get;update;patch

func (r *AlarmContactGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("alarmcontactgroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AlarmContactGroup")
...
func (r *AlarmContactGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cmsv1alpha1.AlarmContactGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", cmsv1alpha1.AlarmContactGroup{}.APIVersion, cmsv1alpha1.AlarmContactGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cmsv1alpha1.AlarmContactGroup{}).
