...
// ChartNamespaceReconciler reconciles a ChartNamespace object
type ChartNamespaceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cr.alicloud.kubeform.com,resources=chartnamespaces,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cr.alicloud.kubeform.com,resources=chartnamespaces/status,verbs=get;update;patch

func (r *ChartNamespaceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("chartnamespace", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ChartNamespace")
...
func (r *ChartNamespaceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &crv1alpha1.ChartNamespace{}); err != nil {
			klog.Error(err, "unable to set up auditor", crv1alpha1.ChartNamespace{}.APIVersion, crv1alpha1.ChartNamespace{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&crv1alpha1.ChartNamespace{}).
