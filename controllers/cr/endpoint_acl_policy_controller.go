...
// EndpointACLPolicyReconciler reconciles a EndpointACLPolicy object
type EndpointACLPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cr.alicloud.kubeform.com,resources=endpointaclpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cr.alicloud.kubeform.com,resources=endpointaclpolicies/status,verbs=get;update;patch

func (r *EndpointACLPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("endpointaclpolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EndpointACLPolicy")
...
func (r *EndpointACLPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &crv1alpha1.EndpointACLPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", crv1alpha1.EndpointACLPolicy{}.APIVersion, crv1alpha1.EndpointACLPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&crv1alpha1.EndpointACLPolicy{}).
