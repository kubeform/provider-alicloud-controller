...
// EeRepoReconciler reconciles a EeRepo object
type EeRepoReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cr.alicloud.kubeform.com,resources=eerepoes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cr.alicloud.kubeform.com,resources=eerepoes/status,verbs=get;update;patch

func (r *EeRepoReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("eerepo", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EeRepo")
...
func (r *EeRepoReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &crv1alpha1.EeRepo{}); err != nil {
			klog.Error(err, "unable to set up auditor", crv1alpha1.EeRepo{}.APIVersion, crv1alpha1.EeRepo{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&crv1alpha1.EeRepo{}).
