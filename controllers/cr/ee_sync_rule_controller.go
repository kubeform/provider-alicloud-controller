...
// EeSyncRuleReconciler reconciles a EeSyncRule object
type EeSyncRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cr.alicloud.kubeform.com,resources=eesyncrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cr.alicloud.kubeform.com,resources=eesyncrules/status,verbs=get;update;patch

func (r *EeSyncRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("eesyncrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EeSyncRule")
...
func (r *EeSyncRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &crv1alpha1.EeSyncRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", crv1alpha1.EeSyncRule{}.APIVersion, crv1alpha1.EeSyncRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&crv1alpha1.EeSyncRule{}).
