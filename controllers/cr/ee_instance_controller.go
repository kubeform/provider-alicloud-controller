...
// EeInstanceReconciler reconciles a EeInstance object
type EeInstanceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cr.alicloud.kubeform.com,resources=eeinstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cr.alicloud.kubeform.com,resources=eeinstances/status,verbs=get;update;patch

func (r *EeInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("eeinstance", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EeInstance")
...
func (r *EeInstanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &crv1alpha1.EeInstance{}); err != nil {
			klog.Error(err, "unable to set up auditor", crv1alpha1.EeInstance{}.APIVersion, crv1alpha1.EeInstance{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&crv1alpha1.EeInstance{}).
...
func (r *EeInstanceReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for EeInstance")
			return nil
		}
		...
		resourceList := &crv1alpha1.EeInstanceList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list EeInstance")
			return nil
		}
