...
// +kubebuilder:rbac:groups=ehpc.alicloud.kubeform.com,resources=jobtemplates,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ehpc.alicloud.kubeform.com,resources=jobtemplates/status,verbs=get;update;patch

func (r *JobTemplateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("jobtemplate", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch JobTemplate")
...
func (r *JobTemplateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ehpcv1alpha1.JobTemplate{}); err != nil {
			klog.Error(err, "unable to set up auditor", ehpcv1alpha1.JobTemplate{}.APIVersion, ehpcv1alpha1.JobTemplate{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ehpcv1alpha1.JobTemplate{}).
