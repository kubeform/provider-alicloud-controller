...
// ProvisioningGroupReconciler reconciles a ProvisioningGroup object
type ProvisioningGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=auto.alicloud.kubeform.com,resources=provisioninggroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=auto.alicloud.kubeform.com,resources=provisioninggroups/status,verbs=get;update;patch

func (r *ProvisioningGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("provisioninggroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ProvisioningGroup")
...
func (r *ProvisioningGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &autov1alpha1.ProvisioningGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", autov1alpha1.ProvisioningGroup{}.APIVersion, autov1alpha1.ProvisioningGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&autov1alpha1.ProvisioningGroup{}).
