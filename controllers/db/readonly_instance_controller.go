...
// ReadonlyInstanceReconciler reconciles a ReadonlyInstance object
type ReadonlyInstanceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=db.alicloud.kubeform.com,resources=readonlyinstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=db.alicloud.kubeform.com,resources=readonlyinstances/status,verbs=get;update;patch

func (r *ReadonlyInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("readonlyinstance", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ReadonlyInstance")
...
func (r *ReadonlyInstanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dbv1alpha1.ReadonlyInstance{}); err != nil {
			klog.Error(err, "unable to set up auditor", dbv1alpha1.ReadonlyInstance{}.APIVersion, dbv1alpha1.ReadonlyInstance{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dbv1alpha1.ReadonlyInstance{}).
