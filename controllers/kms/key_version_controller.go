...
// KeyVersionReconciler reconciles a KeyVersion object
type KeyVersionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=kms.alicloud.kubeform.com,resources=keyversions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kms.alicloud.kubeform.com,resources=keyversions/status,verbs=get;update;patch

func (r *KeyVersionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("keyversion", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch KeyVersion")
...
func (r *KeyVersionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kmsv1alpha1.KeyVersion{}); err != nil {
			klog.Error(err, "unable to set up auditor", kmsv1alpha1.KeyVersion{}.APIVersion, kmsv1alpha1.KeyVersion{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kmsv1alpha1.KeyVersion{}).
