...
// CiphertextReconciler reconciles a Ciphertext object
type CiphertextReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=kms.alicloud.kubeform.com,resources=ciphertexts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kms.alicloud.kubeform.com,resources=ciphertexts/status,verbs=get;update;patch

func (r *CiphertextReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ciphertext", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Ciphertext")
...
func (r *CiphertextReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kmsv1alpha1.Ciphertext{}); err != nil {
			klog.Error(err, "unable to set up auditor", kmsv1alpha1.Ciphertext{}.APIVersion, kmsv1alpha1.Ciphertext{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kmsv1alpha1.Ciphertext{}).
...
func (r *CiphertextReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for Ciphertext")
			return nil
		}
		...
		resourceList := &kmsv1alpha1.CiphertextList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list Ciphertext")
			return nil
		}
