...
// +kubebuilder:rbac:groups=kms.alicloud.kubeform.com,resources=secrets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kms.alicloud.kubeform.com,resources=secrets/status,verbs=get;update;patch
...
func (r *SecretReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kmsv1alpha1.Secret{}); err != nil {
			klog.Error(err, "unable to set up auditor", kmsv1alpha1.Secret{}.APIVersion, kmsv1alpha1.Secret{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kmsv1alpha1.Secret{}).
...
func (r *SecretReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for Secret")
			return nil
		}
		...
		resourceList := &kmsv1alpha1.SecretList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list Secret")
			return nil
		}
