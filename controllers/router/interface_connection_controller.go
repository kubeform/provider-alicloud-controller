...
// InterfaceConnectionReconciler reconciles a InterfaceConnection object
type InterfaceConnectionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=router.alicloud.kubeform.com,resources=interfaceconnections,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=router.alicloud.kubeform.com,resources=interfaceconnections/status,verbs=get;update;patch

func (r *InterfaceConnectionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("interfaceconnection", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch InterfaceConnection")
...
func (r *InterfaceConnectionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &routerv1alpha1.InterfaceConnection{}); err != nil {
			klog.Error(err, "unable to set up auditor", routerv1alpha1.InterfaceConnection{}.APIVersion, routerv1alpha1.InterfaceConnection{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&routerv1alpha1.InterfaceConnection{}).
