...
// +kubebuilder:rbac:groups=router.alicloud.kubeform.com,resources=interfaces,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=router.alicloud.kubeform.com,resources=interfaces/status,verbs=get;update;patch

func (r *InterfaceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("interface", req.NamespacedName)
...
func (r *InterfaceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &routerv1alpha1.Interface{}); err != nil {
			klog.Error(err, "unable to set up auditor", routerv1alpha1.Interface{}.APIVersion, routerv1alpha1.Interface{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&routerv1alpha1.Interface{}).
