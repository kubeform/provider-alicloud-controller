...
// GatewayGatewayReconciler reconciles a GatewayGateway object
type GatewayGatewayReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=database.alicloud.kubeform.com,resources=gatewaygateways,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=database.alicloud.kubeform.com,resources=gatewaygateways/status,verbs=get;update;patch

func (r *GatewayGatewayReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("gatewaygateway", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GatewayGateway")
...
func (r *GatewayGatewayReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databasev1alpha1.GatewayGateway{}); err != nil {
			klog.Error(err, "unable to set up auditor", databasev1alpha1.GatewayGateway{}.APIVersion, databasev1alpha1.GatewayGateway{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&databasev1alpha1.GatewayGateway{}).
