...
// ApplicationScalingRuleReconciler reconciles a ApplicationScalingRule object
type ApplicationScalingRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=sae.alicloud.kubeform.com,resources=applicationscalingrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sae.alicloud.kubeform.com,resources=applicationscalingrules/status,verbs=get;update;patch

func (r *ApplicationScalingRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("applicationscalingrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ApplicationScalingRule")
...
func (r *ApplicationScalingRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &saev1alpha1.ApplicationScalingRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", saev1alpha1.ApplicationScalingRule{}.APIVersion, saev1alpha1.ApplicationScalingRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&saev1alpha1.ApplicationScalingRule{}).
