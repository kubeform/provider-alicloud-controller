...
// RuleAttachmentReconciler reconciles a RuleAttachment object
type RuleAttachmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=pvtz.alicloud.kubeform.com,resources=ruleattachments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=pvtz.alicloud.kubeform.com,resources=ruleattachments/status,verbs=get;update;patch

func (r *RuleAttachmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ruleattachment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RuleAttachment")
...
func (r *RuleAttachmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &pvtzv1alpha1.RuleAttachment{}); err != nil {
			klog.Error(err, "unable to set up auditor", pvtzv1alpha1.RuleAttachment{}.APIVersion, pvtzv1alpha1.RuleAttachment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&pvtzv1alpha1.RuleAttachment{}).
