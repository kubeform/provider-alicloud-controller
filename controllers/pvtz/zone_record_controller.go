...
// +kubebuilder:rbac:groups=pvtz.alicloud.kubeform.com,resources=zonerecords,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=pvtz.alicloud.kubeform.com,resources=zonerecords/status,verbs=get;update;patch

func (r *ZoneRecordReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("zonerecord", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ZoneRecord")
...
func (r *ZoneRecordReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &pvtzv1alpha1.ZoneRecord{}); err != nil {
			klog.Error(err, "unable to set up auditor", pvtzv1alpha1.ZoneRecord{}.APIVersion, pvtzv1alpha1.ZoneRecord{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&pvtzv1alpha1.ZoneRecord{}).
