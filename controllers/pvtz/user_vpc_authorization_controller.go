...
// UserVpcAuthorizationReconciler reconciles a UserVpcAuthorization object
type UserVpcAuthorizationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=pvtz.alicloud.kubeform.com,resources=uservpcauthorizations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=pvtz.alicloud.kubeform.com,resources=uservpcauthorizations/status,verbs=get;update;patch

func (r *UserVpcAuthorizationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("uservpcauthorization", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch UserVpcAuthorization")
...
func (r *UserVpcAuthorizationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &pvtzv1alpha1.UserVpcAuthorization{}); err != nil {
			klog.Error(err, "unable to set up auditor", pvtzv1alpha1.UserVpcAuthorization{}.APIVersion, pvtzv1alpha1.UserVpcAuthorization{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&pvtzv1alpha1.UserVpcAuthorization{}).
