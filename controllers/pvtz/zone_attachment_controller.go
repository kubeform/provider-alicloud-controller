...
// ZoneAttachmentReconciler reconciles a ZoneAttachment object
type ZoneAttachmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=pvtz.alicloud.kubeform.com,resources=zoneattachments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=pvtz.alicloud.kubeform.com,resources=zoneattachments/status,verbs=get;update;patch

func (r *ZoneAttachmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("zoneattachment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ZoneAttachment")
...
func (r *ZoneAttachmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &pvtzv1alpha1.ZoneAttachment{}); err != nil {
			klog.Error(err, "unable to set up auditor", pvtzv1alpha1.ZoneAttachment{}.APIVersion, pvtzv1alpha1.ZoneAttachment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&pvtzv1alpha1.ZoneAttachment{}).
