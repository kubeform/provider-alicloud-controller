...
// RealTimeLogDeliveryReconciler reconciles a RealTimeLogDelivery object
type RealTimeLogDeliveryReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cdn.alicloud.kubeform.com,resources=realtimelogdeliveries,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cdn.alicloud.kubeform.com,resources=realtimelogdeliveries/status,verbs=get;update;patch

func (r *RealTimeLogDeliveryReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("realtimelogdelivery", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RealTimeLogDelivery")
...
func (r *RealTimeLogDeliveryReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cdnv1alpha1.RealTimeLogDelivery{}); err != nil {
			klog.Error(err, "unable to set up auditor", cdnv1alpha1.RealTimeLogDelivery{}.APIVersion, cdnv1alpha1.RealTimeLogDelivery{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cdnv1alpha1.RealTimeLogDelivery{}).
