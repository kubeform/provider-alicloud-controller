...
// DomainNewReconciler reconciles a DomainNew object
type DomainNewReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cdn.alicloud.kubeform.com,resources=domainnews,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cdn.alicloud.kubeform.com,resources=domainnews/status,verbs=get;update;patch

func (r *DomainNewReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("domainnew", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DomainNew")
...
func (r *DomainNewReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cdnv1alpha1.DomainNew{}); err != nil {
			klog.Error(err, "unable to set up auditor", cdnv1alpha1.DomainNew{}.APIVersion, cdnv1alpha1.DomainNew{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cdnv1alpha1.DomainNew{}).
...
func (r *DomainNewReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for DomainNew")
			return nil
		}
		...
		resourceList := &cdnv1alpha1.DomainNewList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list DomainNew")
			return nil
		}
