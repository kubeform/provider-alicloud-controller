...
// TableAttachmentReconciler reconciles a TableAttachment object
type TableAttachmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=route.alicloud.kubeform.com,resources=tableattachments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=route.alicloud.kubeform.com,resources=tableattachments/status,verbs=get;update;patch

func (r *TableAttachmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("tableattachment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TableAttachment")
...
func (r *TableAttachmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &routev1alpha1.TableAttachment{}); err != nil {
			klog.Error(err, "unable to set up auditor", routev1alpha1.TableAttachment{}.APIVersion, routev1alpha1.TableAttachment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&routev1alpha1.TableAttachment{}).
