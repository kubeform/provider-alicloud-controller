...
// DomainResourceReconciler reconciles a DomainResource object
type DomainResourceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ddoscoo.alicloud.kubeform.com,resources=domainresources,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ddoscoo.alicloud.kubeform.com,resources=domainresources/status,verbs=get;update;patch

func (r *DomainResourceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("domainresource", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DomainResource")
...
func (r *DomainResourceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ddoscoov1alpha1.DomainResource{}); err != nil {
			klog.Error(err, "unable to set up auditor", ddoscoov1alpha1.DomainResource{}.APIVersion, ddoscoov1alpha1.DomainResource{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ddoscoov1alpha1.DomainResource{}).
