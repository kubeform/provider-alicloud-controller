...
// SchedulerRuleReconciler reconciles a SchedulerRule object
type SchedulerRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ddoscoo.alicloud.kubeform.com,resources=schedulerrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ddoscoo.alicloud.kubeform.com,resources=schedulerrules/status,verbs=get;update;patch

func (r *SchedulerRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("schedulerrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SchedulerRule")
...
func (r *SchedulerRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ddoscoov1alpha1.SchedulerRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", ddoscoov1alpha1.SchedulerRule{}.APIVersion, ddoscoov1alpha1.SchedulerRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ddoscoov1alpha1.SchedulerRule{}).
