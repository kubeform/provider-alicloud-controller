...
// BandwidthPackageReconciler reconciles a BandwidthPackage object
type BandwidthPackageReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=common.alicloud.kubeform.com,resources=bandwidthpackages,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=common.alicloud.kubeform.com,resources=bandwidthpackages/status,verbs=get;update;patch

func (r *BandwidthPackageReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("bandwidthpackage", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BandwidthPackage")
...
func (r *BandwidthPackageReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &commonv1alpha1.BandwidthPackage{}); err != nil {
			klog.Error(err, "unable to set up auditor", commonv1alpha1.BandwidthPackage{}.APIVersion, commonv1alpha1.BandwidthPackage{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&commonv1alpha1.BandwidthPackage{}).
